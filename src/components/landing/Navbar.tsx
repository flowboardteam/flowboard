...
import { Menu, X, Search, User, ChevronRight, LayoutDashboard, LogIn, UserPlus, CreditCard } from "lucide-react";
...
export function Navbar({ heroHeight = 700 }: NavbarProps) {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ${
      isOverHero ? "bg-transparent" : "bg-white/80 backdrop-blur-md shadow-sm border-b border-slate-200"
    }`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between py-4 lg:py-5">
          ...
          <div className="flex items-center gap-4 md:gap-6">
            ...
            >
              {isMobileMenuOpen ? <X className={iconColor} /> : <Menu className={iconColor} />}
            </button>
            ...
            <a href="/" className="flex items-center gap-2 group">
              ...
              <span className={`text-xl font-black italic uppercase tracking-tighter hidden md:block ${textColor}`}>
                Flowboard
              </span>
            </a>

            <div className={`hidden lg:block w-px h-6 ${isOverHero ? "bg-white/20" : "bg-gray-200"}`} />
          ...
          <div className="flex items-center gap-3 relative" ref={dropdownRef}>
            ...
            <button onClick={() => setIsSearchOpen(!isSearchOpen)} className={`p-2.5 rounded-full ${hoverBg} transition hidden sm:block`}>
              ...
              {isAuthDropdownOpen && (
                ...
                >
                  <div className="space-y-8">
                    <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Clients</p>
                      <div className="space-y-1">
                        <AuthLink label="Client Login" icon={LogIn} onClick={() => navigateTo("/client/login")} />
                        <AuthLink label="Hire Talent" icon={UserPlus} onClick={() => navigateTo("/client/signup")} />
                      </div>
                    </div>

                    <div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Talent</p>
                      <div className="space-y-1">
                        <AuthLink label="Talent Login" icon={LogIn} onClick={() => navigateTo("/talent/login")} />
                        <AuthLink label="Join Registry" icon={UserPlus} onClick={() => navigateTo("/talent/signup")} />
                        <AuthLink label="Flowboard Pay" icon={CreditCard} onClick={() => navigateTo("/talent/pay")} />
                      </div>
                    </div>

                    {user && (
                      <div className="pt-4 border-t border-slate-100">
                        <AuthLink label="Command Center" icon={LayoutDashboard} onClick={() => navigateTo("/talent/dashboard")} />
                      </div>
                    )}
                  </div>
                </motion.div>
