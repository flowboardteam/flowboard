...
import { Check, ArrowUpRight, Zap, CreditCard, Cloud, Search, MessageSquare, Send, Sparkles } from "lucide-react";
...
const tabs = [
  {
    ...
    description: "We combine expert local recruiters with AI-powered screening to deliver elite global talent, faster.",
    features: [
      { text: "Instant matches", sub: "Get candidate shortlists in days, not weeks" },
      { text: "Quality guaranteed", sub: "Expert recruiters review all AI submissions" },
      { text: "Hire globally", sub: "Tap into a curated talent pool across 100+ countries" }
    ],
  },
  {
    id: "payroll",
    ...
    description: "Automate cross-border payments, tax compliance, and benefits for your entire international team.",
    features: [
      { text: "Local currency payments", sub: "Pay your team in their preferred local currency" },
      { text: "Automated compliance", sub: "Localized contracts and automatic tax filings" },
      { text: "Consolidated billing", sub: "One single invoice for your entire global workforce" }
    ],
  },
  {
    id: "talentcloud",
    ...
    description: "A unified platform to track performance, manage time-off, and scale your culture globally.",
    features: [
      { text: "Performance tracking", sub: "Monitor velocity and output in real-time" },
      { text: "Unified dashboard", sub: "One source of truth for all HR operations" },
      { text: "Agentic Workflows", sub: "AI agents that handle onboarding and offboarding" }
    ],
  },
  {
    id: "assistant",
    ...
    description: "Deploy custom AI agents that understand your company's context and handle complex operations autonomously.",
    features: [
      { text: "Haraka-o1 Reasoning", sub: "Advanced logic for complex decision making" },
      { text: "Context Aware", sub: "Trained on your specific workflows and docs" },
      { text: "Autonomous Action", sub: "Agents that can execute tasks, not just answer questions" }
    ],
  }
];
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <div className="inline-flex items-center gap-2 text-blue-600 mb-6">
                    ...
                    <span className="font-bold uppercase tracking-widest text-xs">{activeTab.label}</span>
                  </div>
                  ...
                  <div className="space-y-5 mb-10">
                    {activeTab.features.map((feature, i) => (
                      <div key={i} className="flex gap-4 items-start">
                        ...
                        <div>
                          <p className="font-bold text-slate-900">{feature.text}.</p>
                  ...
                  <Button variant="outline" className="h-12 px-8 rounded-xl border-slate-200 font-bold group">
                    Explore {activeTab.label} <ArrowUpRight className="ml-2 w-4 h-4 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" />
                  </Button>
                </motion.div>
              </AnimatePresence>
            </div>
    ...
    <div className="w-full max-w-[400px] bg-white rounded-3xl shadow-2xl border border-slate-100 flex flex-col overflow-hidden h-[400px]">
      <div className="p-4 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">Haraka-o1 Active</span>
        </div>
      ...
      <div className="flex-1 p-6 space-y-4 overflow-y-auto">
        <div className="bg-slate-50 rounded-2xl rounded-tl-none p-3 text-[11px] text-slate-600 max-w-[80%]">
          Hello! I've analyzed the Q4 payroll data. Would you like me to generate the compliance reports for Brazil?
        </div>
        <div className="bg-blue-600 rounded-2xl rounded-tr-none p-3 text-[11px] text-white ml-auto max-w-[80%]">
          Yes, please. Also check if the new engineering hires in Poland are verified.
        </div>
      ...
      <div className="p-4 border-t border-slate-50">
        <div className="flex gap-2 items-center bg-slate-50 px-4 py-2 rounded-full border border-slate-100">
          <span className="text-[11px] text-slate-400 flex-1">Ask Haraka anything...</span>
  ...
  return (
    <div className="w-full max-w-[400px] bg-white rounded-2xl shadow-2xl border border-slate-100 p-6 relative overflow-hidden">
      ...
      <div className="flex justify-center items-end gap-1 mb-10 h-24">
        {talents.map((t, i) => (
          <motion.div key={t.id} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: i * 0.1 }}>
            <div className="w-14 h-14 rounded-full border-4 border-white shadow-lg overflow-hidden bg-slate-100">
              <img src={t.img} alt="talent" className="w-full h-full object-cover" />
            </div>
            <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-white rounded-full shadow-sm flex items-center justify-center text-xs">{t.flag}</div>
          </motion.div>
        ))}
      </div>
      <Button className="w-full bg-blue-600 hover:bg-blue-700 h-12 rounded-xl font-bold shadow-lg shadow-blue-200">Search</Button>
    </div>
  );
}
  ...
  return (
    <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl border border-slate-100 p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">Syncing</span>
      </div>
      <div className="space-y-3">
        {[ { n: 'Mateo Garcia', a: '$4,200', c: 'USD' }, { n: 'Aisha Rahman', a: '$3,850', c: 'GBP' } ].map(p => (
          <div key={p.n} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
    ...
    <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl border border-slate-100 p-6">
       <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
             <p className="text-[10px] font-bold text-blue-600 uppercase mb-1">Velocity</p>
       ...
       <div className="p-5 bg-blue-600 rounded-2xl text-white">
          <p className="text-[10px] font-bold uppercase opacity-80 mb-3">Model Accuracy</p>
