...
export function TalentCard({ talent, onReview }: TalentCardProps) {
  ...
  return (
    <div className="bg-[var(--sidebar-bg)] border border-[var(--border-color)] rounded-2xl p-4 sm:p-6 hover:border-emerald-500/50 transition-all group relative flex flex-col h-full shadow-sm hover:shadow-xl hover:shadow-emerald-500/5">
      <div className="flex justify-between items-start mb-4 sm:mb-6">
        <div className="flex gap-3 sm:gap-4 overflow-hidden">
          {/* Fixed Avatar Container */}
          <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-xl bg-slate-100 dark:bg-slate-800/50 border border-[var(--border-color)] flex items-center justify-center overflow-hidden shrink-0 shadow-inner">
      ...
      <div className="space-y-4 sm:space-y-5 flex-1 flex flex-col justify-between">
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {!talent.skills?.length && <span className="text-[9px] sm:text-[10px] text-slate-400">General Expertise</span>}
          </div>
        </div>
