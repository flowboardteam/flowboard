...
export function TalentCard({ talent, onReview }: TalentCardProps) {
  ...
  return (
    <div className="bg-[var(--sidebar-bg)] border border-[var(--border-color)] rounded-2xl sm:rounded-3xl p-4 sm:p-6 hover:border-emerald-500/50 transition-all group relative flex flex-col h-full shadow-sm hover:shadow-xl hover:shadow-emerald-500/5">
      <div className="flex justify-between items-start mb-4 sm:mb-6">
        <div className="flex gap-3 sm:gap-4 overflow-hidden">
          {/* Fixed Avatar Container */}
          <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-slate-100 dark:bg-slate-800/50 border border-[var(--border-color)] flex items-center justify-center overflow-hidden shrink-0 shadow-inner">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {talent.skills?.slice(0, 3).map((skill: string) => (
              <span key={skill} className="px-2 sm:px-2.5 py-1 bg-emerald-500/5 dark:bg-emerald-500/10 rounded-md text-[9px] sm:text-[10px] font-black uppercase text-emerald-600 dark:text-emerald-400 border border-emerald-500/10 dark:border-emerald-500/20">
                {skill}
              </span>
            ))}
