  ...
  return (
    <aside className="w-full h-full bg-[var(--sidebar-bg)] border-r border-[var(--border-color)] flex flex-col p-6 overflow-y-auto no-scrollbar relative">
      ...
      <div className="flex items-center gap-3 mb-10 px-2">
        <div className="w-10 h-10 rounded-none bg-emerald-500/10 flex items-center justify-center border border-emerald-500/20 shadow-sm">
        ...
        {menu.map(item => {
            ...
            return (
              <div key={item.group} className="space-y-1">
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-3 rounded-none text-slate-500 hover:bg-emerald-500/5 hover:text-emerald-600 transition-all font-bold text-sm group"
                >
                    ...
                    {item.items?.map(sub => {
                      ...
                      return (
                        <NavLink
                          ...
                          onClick={onClose}
                          className={({ isActive }) =>
                            `flex items-center justify-between px-4 py-2.5 rounded-none transition-all font-semibold text-[13px] ${
                              isActive
                        ...
                        >
                          ...
                          {badge !== null && (
                            <span className={`text-[10px] font-black px-2 py-0.5 rounded-none min-w-[20px] text-center shrink-0 ${badgeColor}`}>
                              {badge}
                            </span>
                          )}
                        </NavLink>
          ...
          return (
            <NavLink
              ...
              onClick={onClose}
              className={({ isActive }) =>
                `flex items-center gap-4 px-4 py-3 rounded-none transition-all font-bold text-sm ${
                  isActive
      ...
      <div className="mt-10 pt-6 border-t border-[var(--border-color)] relative">
        {showMiniMenu && (
          <div className="absolute bottom-full left-0 mb-4 w-full bg-[var(--sidebar-bg)] border border-[var(--border-color)] rounded-none shadow-2xl p-2 z-50 ring-1 ring-black/5">
            <button onClick={() => { navigate("/client/profile"); setShowMiniMenu(false); }}
              className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black tracking-widest text-slate-500 hover:bg-emerald-500/5 hover:text-emerald-600 rounded-none transition-all">
            ...
            <button onClick={handleLogout}
              className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black tracking-widest text-red-500 hover:bg-red-500/10 rounded-none transition-all">
        ...
        <button
          ...
          className="w-full flex items-center justify-between p-2 rounded-none hover:bg-slate-500/5 transition-all"
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-none bg-slate-200 dark:bg-slate-800 border border-[var(--border-color)] flex items-center justify-center overflow-hidden shadow-sm shrink-0">
              {profile?.avatar_url
