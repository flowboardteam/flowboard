  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-1 bg-gradient-to-r from-transparent via-slate-400 to-transparent opacity-50" />

        <div className="text-center mb-10">
          <div className="w-16 h-16 bg-slate-100 rounded-xl flex items-center justify-center mx-auto mb-6 border border-slate-200">
            ...
            return (
              <div 
                ...
                className={`flex items-center gap-4 p-4 rounded-xl border transition-all duration-500 ${
                  isActive ? "bg-slate-50 border-[#1A1C21]/30" : 
                  isCompleted ? "bg-slate-50 border-slate-200 opacity-50" : 
                  "bg-white/5 border-transparent opacity-20"
                }`}
              >
                <div className={`${isCompleted ? "text-slate-900" : isActive ? "text-[#1A1C21]" : "text-slate-400"}`}>
                ...
                {isActive && (
                   <div className="ml-auto flex gap-1">
                      <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1 }} className="w-1 h-1 bg-[#1A1C21] rounded-full" />
                      <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1, delay: 0.2 }} className="w-1 h-1 bg-[#1A1C21] rounded-full" />
                      <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1, delay: 0.4 }} className="w-1 h-1 bg-[#1A1C21] rounded-full" />
                   </div>
                )}
              </div>
            );
        ...
        <div className="mt-10 p-4 rounded-xl bg-black/40 border border-white/5 font-mono text-[9px] text-[#1A1C21]/60 leading-relaxed overflow-hidden">
