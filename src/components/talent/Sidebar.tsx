  ...
  return (
    <aside className="w-full h-full bg-[var(--sidebar-bg)] border-r border-[var(--border-color)] flex flex-col p-6 overflow-y-auto">
      <div className="flex items-center gap-3 mb-10 px-2">
        ...
        <span className="text-lg font-medium tracking-tight text-[var(--text-main)]">Talent</span>
      </div>

      <nav className="flex-1 space-y-8">
        {MENU_GROUPS.map(group => (
          <div key={group.group}>
            <p className="text-[10px] font-medium uppercase tracking-wider text-slate-400 mb-3 px-4">
              {group.group}
            </p>
              ...
              {group.items.map(item => {
                ...
                return (
                  ...
                  >
                    ...
                    {badgeCount > 0 && (
                      <span className="ml-2 bg-amber-500 text-white text-[10px] font-medium px-2 py-0.5 rounded-lg min-w-[20px] text-center shrink-0 shadow-md shadow-amber-500/30">
                        {badgeCount}
                      </span>
                    )}
                  </NavLink>
