  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <motion.div 
        ...
        className="relative w-full max-w-lg bg-[var(--card-bg)] border border-[var(--border-color)] rounded-none p-8 md:p-12 overflow-hidden"
      >
        ...
        <div className="text-center mb-10">
          <div className="w-16 h-16 bg-blue-500/10 rounded-none flex items-center justify-center mx-auto mb-6 border border-blue-500/20">
            <Loader2 className="w-8 h-8 text-blue-500 animate-spin" />
          </div>
          <h2 className="text-2xl font-black italic uppercase tracking-tighter text-[var(--foreground)]">
            Deploying <span className="text-blue-500 text-nowrap">Mission Node</span>
          </h2>
        ...
        <div className="space-y-4">
          {STEPS.map((step, idx) => {
            ...
            return (
              <div 
                key={step.id}
                className={`flex items-center gap-4 p-4 rounded-none border transition-all duration-500 ${
              ...
              >
                ...
                {isActive && (
                   <div className="ml-auto flex gap-1">
                      <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1 }} className="w-1 h-1 bg-blue-500 rounded-none" />
                      <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1, delay: 0.2 }} className="w-1 h-1 bg-blue-500 rounded-none" />
                      <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1, delay: 0.4 }} className="w-1 h-1 bg-blue-500 rounded-none" />
                   </div>
                )}
              </div>
            );
        ...
        <div className="mt-10 p-4 rounded-none bg-black/40 border border-white/5 font-mono text-[9px] text-blue-400/60 leading-relaxed overflow-hidden">
