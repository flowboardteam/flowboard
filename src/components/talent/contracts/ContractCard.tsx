...
export default function ContractCard({ contract, viewMode = 'list' }: any) {
  return (
    <div className={`
      ...
      p-5 md:p-6 
      ${viewMode === 'list' ? 'rounded-none md:rounded-none' : 'rounded-none md:rounded-none flex flex-col h-full'}
    `}>
      ...
      <div className={`relative z-10 flex ${viewMode === 'list' ? 'flex-col md:flex-row md:items-center' : 'flex-col'} justify-between gap-4 md:gap-6`}>
        ...
        <div className="flex gap-4 items-start md:items-center">
          <div className="w-12 h-12 md:w-14 md:h-14 rounded-none md:rounded-none bg-white/5 border border-[var(--border-color)] p-2.5 shrink-0 flex items-center justify-center group-hover:scale-110 transition-transform">
            <img src={contract.orgLogo} alt="Logo" className="w-full h-full object-contain" />
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
               <span className="text-[7px] md:text-[8px] font-black uppercase tracking-widest text-blue-500 bg-blue-500/10 px-1.5 py-0.5 rounded-none">
                 {contract.id}
               </span>
            ...
            <h3 className="text-lg md:text-xl font-black italic uppercase tracking-tighter text-[var(--foreground)] group-hover:text-blue-400 transition-colors truncate">
              {contract.role}
            </h3>
          </div>
        </div>
        ...
        `}>
          <div className="text-left md:text-right">
             <div className="flex items-center md:justify-end gap-1 text-emerald-500">
               ...
               <span className="text-xs md:text-sm font-black italic">{contract.aiMatchScore}%</span>
             </div>
             <p className="text-[7px] md:text-[8px] font-black uppercase tracking-widest text-slate-500">Match Sync</p>
          </div>
          
          <div className="text-left md:text-right">
             <p className="text-xs md:text-sm font-black italic text-[var(--foreground)]">${contract.rate}</p>
          ...
          <div className={`
            hidden md:flex items-center justify-center w-10 h-10 rounded-none bg-white/5 border border-[var(--border-color)] text-slate-400 
      ...
      `}>
        <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1 md:pb-0">
          {contract.skills.slice(0, 3).map((skill: string) => (
            <span key={skill} className="whitespace-nowrap text-[8px] md:text-[9px] font-black uppercase tracking-widest text-slate-500 border border-[var(--border-color)] px-2 py-1 rounded-none bg-white/5">
              {skill}
            </span>
          ))}
        </div>
