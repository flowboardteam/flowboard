...
export default function SignUp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        // 1. Check if it's actually an "Existing User" error
        if (
        ...
        // 2. Otherwise, throw the actual error (e.g., "Password should be at least 6 characters")
        throw error;
      }

      // If Supabase returns data but identities is empty, it's an existing user
      // Note: Some Supabase versions ignore this if Enumeration Protection is ON.
      const isExisting =
      ...
      if (isExisting) {
        ...
        setNotification({
          ...
          description:
            "We've sent a link to your inbox. Please click it to activate your account.",
        });
        ...
        <div className="hidden lg:flex flex-col justify-between bg-[#050B1E] p-16 text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            >
              <h2 className="text-5xl font-extrabold leading-[1.1] mb-8 tracking-tight">
                Bridge the <span className="text-blue-400">skill gap</span>{" "}
                <br />
          ...
          >
            <p className="text-lg text-blue-100/90 leading-relaxed mb-6 font-medium italic">
              "Flowboard eliminated the friction in our hiring cycle. We found
              our lead AI researcher in days, not months."
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <AnimatePresence mode="wait">
                {role === "recruiter" ? (
                  ...
                  >
                    ...
                    <Input
                      id={role === "recruiter" ? "company" : "fullname"}
                      value={nameOrCompany}
                      onChange={(e) => setNameOrCompany(e.target.value)}
                      placeholder={
                        role === "recruiter" ? "e.g. Nexus AI Labs" : "John Doe"
                      }
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="email"
                  type="email"
                  value={email} // Fix: Use 'email' state here
                  onChange={(e) => setEmail(e.target.value)} // Fix: Use 'setEmail' here
                  ...
                  <div className="space-y-1.5 pt-1 px-1">
                    <div className="flex gap-1 h-1">
                      {[1, 2, 3, 4].map((step) => (
                        <div
                          key={step}
                          className={`h-full flex-1 rounded-full transition-colors duration-500 ${passwordScore >= step ? strengthColors[passwordScore] : "bg-slate-100"}`}
                        />
                      ))}
                    </div>
                    <p
                      className={`text-[10px] font-bold uppercase tracking-wider ${passwordScore <= 1 ? "text-rose-500" : "text-slate-400"}`}
                    >
