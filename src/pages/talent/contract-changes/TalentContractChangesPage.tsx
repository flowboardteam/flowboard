  ...
  return(
    <div className="space-y-2">
      {changes.map(c=>{
        ...
        return(
          <div key={c.key} className="flex items-center gap-3 p-3 rounded-xl bg-slate-500/5 border border-[var(--border-color)]">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {c.from&&<span className="text-[10px] font-black bg-slate-500/10 text-slate-500 px-2 py-0.5 rounded-lg border border-[var(--border-color)] line-through opacity-60">{c.key==="member_type"?TYPE_LABELS[c.from]??c.from:c.from}</span>}
                {c.from&&<ArrowRightLeft className="w-3 h-3 text-slate-400 shrink-0"/>}
                <span className={`text-[10px] font-black px-2 py-0.5 rounded-lg border ${c.key==="member_type"?TYPE_COLORS[c.to??""]??"bg-blue-500/10 text-blue-600 border-blue-500/20":"bg-blue-500/10 text-blue-600 border-blue-500/20"}`}>
  ...
  return(
    <motion.div layout initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.97}}
      ...
      <div className="flex gap-2 pt-2 border-t border-[var(--border-color)]">
        <button onClick={()=>onView(req)} className="flex-1 py-2 bg-slate-500/10 text-slate-500 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-500/20 transition-all">
          View details
        </button>
        {isPending&&(
          <>
            <button onClick={()=>onRespond(req,"accepted")} className="flex items-center justify-center gap-1 px-3 py-2 bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-emerald-400 transition-all shadow-sm shadow-emerald-500/20 shrink-0">
              <CheckCircle2 className="w-3 h-3"/><span className="hidden sm:inline">Accept</span>
            </button>
            <button onClick={()=>onRespond(req,"declined")} className="flex items-center justify-center gap-1 px-3 py-2 bg-red-500/10 text-red-500 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-red-500/20 transition-all border border-red-500/20 shrink-0">
    ...
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose}
        ...
        <motion.div initial={{opacity:0,y:40}} animate={{opacity:1,y:0}} exit={{opacity:0,y:40}}
          ...
          {/* Drag handle */}
          <div className="flex justify-center pt-3 pb-1 sm:hidden"><div className="w-10 h-1 rounded-full bg-slate-500/30"/></div>
          ...
          <div className="px-6 pt-4 pb-4 border-b border-[var(--border-color)]">
            <div className="flex items-start justify-between gap-3 mb-3">
              ...
              <button onClick={onClose} className="p-2 rounded-xl hover:bg-slate-500/10 text-slate-400 flex-shrink-0"><X className="w-4 h-4"/></button>
            </div>
            <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full border ${cfg.bg} ${cfg.color}`}>
            ...
            {isPending?(
              showDecline?(
                <div className="flex gap-2">
                  <button onClick={()=>setShowDecline(false)} className="flex-1 py-3 border border-[var(--border-color)] text-[10px] font-black uppercase tracking-widest text-slate-500 rounded-xl hover:bg-slate-500/5 transition-all">Back</button>
                ...
                <div className="flex gap-2">
                  <button onClick={()=>setShowDecline(true)} className="flex-1 py-3 border border-red-500/20 bg-red-500/10 text-red-500 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-red-500/20 transition-all">Decline</button>
              ...
              <button onClick={onClose} className="w-full py-3 border border-[var(--border-color)] text-[10px] font-black uppercase tracking-widest text-slate-500 rounded-xl hover:bg-slate-500/5 transition-all">Close</button>
            )}
          </div>
        </motion.div>
      </motion.div>
  ...
  return(
    <div className="w-full space-y-6 pb-20 p-4 sm:p-6 lg:p-8">
      ...
      <div className="space-y-1">
        ...
        <h1 className="text-3xl sm:text-4xl font-extrabold dark:text-white tracking-tight">Contract <span className="text-blue-600">changes.</span></h1>
      ...
      <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1">
        {TABS.map(tab=>(
          <button key={tab} onClick={()=>setActiveTab(tab)}
            className={`flex-shrink-0 px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border ${activeTab===tab?"bg-[var(--card-bg)] text-blue-600 border-blue-500/20 shadow-sm":"border-transparent text-slate-400 hover:text-slate-600"}`}>
      ...
      <AnimatePresence mode="popLayout">
        {filtered.length>0?(
          ...
          <motion.div initial={{opacity:0}} animate={{opacity:1}} className="py-24 text-center space-y-4 bg-slate-500/5 rounded-[2rem] border border-dashed border-[var(--border-color)]">
