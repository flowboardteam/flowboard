...
function SourcingTalentCard({ talent, role, isShortlisted, onShortlist, onReview }) {
  ...
  return (
    <motion.div
      ...
      className={`p-6 rounded-[2rem] bg-[var(--card-bg)] border transition-all flex flex-col relative ${
        score >= 80
    ...
    >
      ...
      <div className="flex items-start gap-4 mb-4 pr-16">
        <div className="w-12 h-12 rounded-2xl bg-slate-500/10 border border-[var(--border-color)] flex items-center justify-center overflow-hidden shrink-0">
          {talent.avatar_url
      ...
      {(matchedSkills.length > 0 || missingSkills.length > 0) && (
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {matchedSkills.map(s => (
              <span key={s} className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wider bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 px-2 py-0.5 rounded-full">
                <CheckCircle2 className="w-2.5 h-2.5" /> {s}
              </span>
            ))}
            {missingSkills.slice(0, 2).map(s => (
              <span key={s} className="text-[10px] font-black uppercase tracking-wider bg-slate-500/10 text-slate-400 px-2 py-0.5 rounded-full">{s}</span>
            ))}
          </div>
        </div>
      )}
        ...
        <div className="p-8">
          <div className="flex items-start justify-between mb-8">
            <div className="flex items-start gap-4">
              <div className="w-16 h-16 rounded-2xl bg-slate-500/10 border border-[var(--border-color)] flex items-center justify-center overflow-hidden shrink-0">
                {talent.avatar_url
                  ? <img src={talent.avatar_url} alt={talent.full_name} className="w-full h-full object-cover" />
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {talent.skills.map(s => {
                  ...
                  return (
                    <span key={s} className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full border ${matched ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" : "bg-slate-500/10 text-slate-500 border-slate-500/10"}`}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-8">
            {[
              ...
              <div key={m.label} className="bg-slate-500/5 rounded-2xl p-3">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
          ...
          {!roleLoading && role && (
            <div className="flex flex-wrap items-center gap-2">
              {[role.department, role.type, role.location, role.experience_level].filter(Boolean).map(tag => (
                <span key={tag} className="text-[10px] font-black uppercase tracking-wider bg-slate-500/10 text-slate-500 px-2.5 py-1 rounded-full">{tag}</span>
              ))}
              {role.skills?.slice(0, 4).map(s => (
                <span key={s} className="text-[10px] font-black uppercase tracking-wider bg-blue-500/10 text-blue-600 px-2.5 py-1 rounded-full">{s}</span>
              ))}
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-shrink-0">
          ...
          >
            ...
            Search deeper
            <span className="text-[8px] bg-emerald-500/10 text-emerald-600 px-1.5 py-0.5 rounded-full border border-emerald-500/20 font-black uppercase tracking-wider">
              Haraka-01
            </span>
          </button>
        ...
        <AnimatePresence mode="popLayout">
          {sorted.length > 0 ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              className="py-24 text-center space-y-5 bg-slate-500/5 rounded-[2.5rem] border border-dashed border-[var(--border-color)]">
