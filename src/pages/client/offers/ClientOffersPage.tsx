...
const STATUS_CONFIG: Record<string, {
  ...
  pending:   { label: "Pending",   color: "text-slate-900",   bg: "bg-slate-100 border-slate-200",     dot: "bg-[#1A1C21]",   icon: Clock        },
  viewed:    { label: "Viewed",    color: "text-[#1A1C21]",    bg: "bg-slate-100 border-slate-200",       dot: "bg-[#1A1C21]",    icon: Clock        },
  accepted:  { label: "Accepted",  color: "text-slate-900", bg: "bg-slate-100 border-slate-200", dot: "bg-[#1A1C21]", icon: CheckCircle2 },
      ...
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onCancel}
        ...
        <motion.div initial={{opacity:0,scale:0.95,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:16}}
          ...
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${danger?"bg-red-500/10":"bg-slate-100"}`}>
            <AlertCircle className={`w-5 h-5 ${danger?"text-red-500":"text-slate-900"}`}/>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={onConfirm} className={`flex-1 py-3 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 ${danger?"bg-red-50 hover:bg-red-400 shadow-md shadow-red-500/20":"bg-[#1A1C21] hover:bg-amber-400"}`}>
              {confirmLabel}
            </button>
          </div>
        </motion.div>
...
function OfferCard({ offer, onView, onWithdraw, onConfirmWithdraw, onWorkforceNavigate }: {
  ...
  return (
    <motion.div
      ...
      className="p-4 sm:p-5 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-color)] flex flex-col gap-3 hover:border-[#1A1C21]/30 transition-all"
    >
      {/* Status + source + date */}
      ...
      <div className="flex items-center gap-3">
        ...
        {offer.status === "accepted" && (
          <span className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-slate-900 bg-slate-100 px-2 py-1 rounded-lg border border-slate-200 shrink-0">
      ...
      <div className="p-3 rounded-xl bg-slate-500/5 border border-[var(--border-color)]">
        <p className="text-xs font-black dark:text-white mb-2 truncate flex items-center gap-1.5">
          <Briefcase className="w-3.5 h-3.5 text-[#1A1C21] shrink-0" />
          {offer.role_title ?? "Role not specified"}
        </p>
        <div className="grid grid-cols-2 gap-x-3 gap-y-1">
          <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400">
            <DollarSign className="w-3 h-3 shrink-0 text-slate-900" />
            <span className="truncate">{fmtCurrency(offer.salary_monthly, offer.salary_currency)}</span>
          </div>
          {offer.role_type && (
            <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400">
              <Briefcase className="w-3 h-3 shrink-0 text-[#1A1C21]" />
          ...
          {offer.start_date && (
            <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 col-span-2">
              <Calendar className="w-3 h-3 shrink-0 text-slate-900" />
      ...
      <div className="flex gap-2 pt-2 border-t border-[var(--border-color)]">
        ...
        {offer.status === "accepted" && onWorkforceNavigate && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2.5 bg-slate-100 text-slate-900 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21]/20 transition-all border border-emerald-500/10 shrink-0"
          >
        ...
        >
          ...
          <div className="px-6 pt-4 pb-4 border-b border-[var(--border-color)] shrink-0">
            ...
            <h3 className="text-xl font-black dark:text-white flex items-center gap-2 tracking-tight">
              <Briefcase className="w-5 h-5 text-[#1A1C21] shrink-0" />
              {offer.role_title ?? "Role not specified"}
            </h3>
          ...
          <div className="px-6 py-6 overflow-y-auto space-y-6 flex-1">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1.5">
                <FileText className="w-3.5 h-3.5 text-slate-900" /> Client Notes / Offer Letter
              </p>
            ...
            {loadingRole ? (
              <div className="flex items-center gap-3 py-4 text-slate-400 text-xs font-bold">
                <Loader2 className="w-4 h-4 animate-spin text-[#1A1C21]" /> Loading attached role specification...
              </div>
            ) : roleDetails ? (
              <div className="space-y-4 pt-4 border-t border-[var(--border-color)]">
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
                  <Award className="w-3.5 h-3.5 text-[#1A1C21]" /> Attached Role Specification
                </p>
                ...
                <div className="space-y-3 bg-[var(--sidebar-bg)] p-5 rounded-2xl border border-[var(--border-color)]">
                  <div className="flex flex-wrap items-center gap-3 text-xs font-bold text-slate-400">
                    ...
                    {roleDetails.location && <span className="flex items-center gap-1"><MapPin className="w-3 h-3 text-slate-900" /> {roleDetails.location}</span>}
                      ...
                      <ul className="space-y-1.5">
                        {roleDetails.responsibilities.map((req: string, idx: number) => (
                          <li key={idx} className="text-xs text-slate-600 dark:text-slate-300 flex items-start gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-[#1A1C21] mt-1.5 shrink-0" />
                            <span>{req}</span>
                          </li>
                        ))}
                      </ul>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {roleDetails.skills.map((skill: string) => (
                          <span key={skill} className="px-2.5 py-1 bg-slate-100 rounded-lg text-[10px] font-black uppercase text-[#1A1C21] dark:text-[#1A1C21] border border-slate-200">
                            {skill}
                          </span>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {roleDetails.benefits.map((b: string, idx: number) => (
                          <span key={idx} className="px-2.5 py-1 bg-slate-100 rounded-lg text-[10px] font-bold text-slate-900 dark:text-emerald-400 border border-slate-200 flex items-center gap-1">
                            <Check className="w-3 h-3 text-slate-900" /> {b}
                          </span>
                        ))}
                      </div>
                    </div>
    ...
    return (
      <div className="w-full flex flex-col items-center justify-center py-32 gap-4">
        ...
        <button onClick={fetchOffers}
          className="flex items-center gap-2 px-5 py-3 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all">
    ...
    <div className="w-full space-y-6 pb-20 p-4 sm:p-6 lg:p-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: "Pending",      value: counts.pending,  color: "text-slate-900"                  },
          { label: "Accepted",     value: counts.accepted, color: "text-slate-900"                },
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-3 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"
        />
      </div>
      ...
      <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1">
        {TABS.map(tab => (
          <button
            ...
            className={`flex-shrink-0 px-4 py-2 rounded-xl text-[11px] font-bold tracking-tight transition-all border ${
              activeTab === tab
                ? "bg-[var(--card-bg)] text-[#1A1C21] border-slate-200 shadow-sm"
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            {!search && activeTab === "all" && (
              <button onClick={() => navigate("/client/talent-pool")}
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#1A1C21] text-white text-[11px] font-bold tracking-tight rounded-xl hover:bg-black transition-all shadow-md shadow-slate-900/10">
