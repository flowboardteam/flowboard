...
function Avatar({member,size="sm"}:{member:WorkforceMember;size?:"sm"|"md"|"lg"}){
  ...
  const sz=size==="lg"?"w-12 h-12 rounded-2xl":size==="md"?"w-10 h-10 rounded-xl":"w-8 h-8 rounded-xl";
  return(
      ...
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onCancel}
        ...
        <motion.div initial={{opacity:0,scale:0.95,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:16}}
          ...
          <div className="flex gap-3">
            ...
            <button onClick={onConfirm} className={`flex-1 py-3 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${danger?"bg-red-500 hover:bg-red-400":"bg-amber-500 hover:bg-amber-400"}`}>{confirmLabel}</button>
          </div>
        </motion.div>
      </motion.div>
    </AnimatePresence>
...
function TagInput({tags,onAdd,onRemove}:{tags:string[];onAdd:(v:string)=>void;onRemove:(v:string)=>void;}){
  ...
  return(
    <div className="min-h-[44px] bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-3 py-2 flex flex-wrap gap-2 items-center focus-within:ring-2 ring-blue-500/20 transition-all cursor-text"
      onClick={e=>(e.currentTarget.querySelector("input") as HTMLInputElement)?.focus()}>
      {tags.map(t=><span key={t} className="inline-flex items-center gap-1 text-[11px] font-black uppercase tracking-wider bg-blue-500/10 text-blue-600 px-2 py-0.5 rounded-full">{t}<button type="button" onClick={()=>onRemove(t)} className="hover:text-red-500 transition-colors"><X className="w-2.5 h-2.5"/></button></span>)}
    ...
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose}
        ...
        <motion.div initial={{opacity:0,scale:0.95,y:20}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95}}
          onClick={e=>e.stopPropagation()}
          className="w-full max-w-md bg-[var(--card-bg)] border border-[var(--border-color)] rounded-[2.5rem] shadow-2xl overflow-hidden">
          ...
          <div className="px-7 pt-7 pb-4">
            ...
            <div className="relative">
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)}
                ...
                className="w-full pl-9 pr-4 py-2.5 bg-slate-500/5 border border-[var(--border-color)] rounded-xl text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/>
            </div>
          </div>
                ...
                <div key={m.id}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {availBadge&&avail!=="available"&&(
                        <span className={`text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-full border ${availBadge}`}>
                          {avail.replace("_"," ")}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="shrink-0">
                    {isAssigned?(
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${isSelected?"bg-blue-600 border-blue-600":"border-slate-300 dark:border-slate-600"}`}>
            ...
            {selected.size>0&&(
              <div>
                ...
                <input value={roleInput} onChange={e=>setRoleInput(e.target.value)}
                  ...
                  className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-2.5 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/>
              </div>
            )}
            <button onClick={handleAssign} disabled={selected.size===0||saving}
              className="w-full py-3.5 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-500 transition-all flex items-center justify-center gap-2 shadow-md shadow-blue-600/20 disabled:opacity-40 disabled:cursor-not-allowed">
  ...
  return(
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose}
        ...
        <motion.div initial={{opacity:0,scale:0.95,y:20}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95}}
          onClick={e=>e.stopPropagation()}
          className="w-full max-w-xl bg-[var(--card-bg)] border border-[var(--border-color)] rounded-[2.5rem] shadow-2xl overflow-hidden">
          ...
          <div className="px-8 pb-0 space-y-4 max-h-[60vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  <select value={form.status} onChange={e=>upd("status",e.target.value)} className="w-full appearance-none bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 cursor-pointer">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {Object.entries(PRIORITY_CFG).map(([k,v])=><button key={k} type="button" onClick={()=>upd("priority",k)} className={`py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all border ${form.priority===k?`${v.bg} ${v.color} border-current`:"border-[var(--border-color)] text-slate-400 hover:bg-slate-500/5"}`}>{v.label}</button>)}
                </div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">End date</label><input type="date" value={form.end_date} onChange={e=>upd("end_date",e.target.value)} className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2"><DollarSign className="inline w-3 h-3 mr-0.5"/>Total budget</label><input value={form.budget} onChange={e=>upd("budget",e.target.value)} type="number" placeholder="e.g. 50000" className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/></div>
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Budget spent</label><input value={form.budget_spent} onChange={e=>upd("budget_spent",e.target.value)} type="number" placeholder="e.g. 12000" className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/></div>
            </div>
            <div>
            ...
            <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-500/5 border border-[var(--border-color)]">
              ...
              <button type="button" onClick={()=>upd("is_client_facing",!form.is_client_facing)}
                className={`w-12 h-6 rounded-full transition-all relative ${form.is_client_facing?"bg-blue-600":"bg-slate-300 dark:bg-slate-600"}`}>
          ...
          <div className="px-8 py-6 flex gap-3">
            ...
            <button onClick={()=>onSave(form)} disabled={!form.name.trim()||saving} className="flex-1 py-3.5 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-500 transition-all flex items-center justify-center gap-2 shadow-md shadow-blue-600/20 disabled:opacity-40">
...
function ProjectCard({project,projectMembers,allMembers,onView,onEdit,onDelete,onAssign}:{
  ...
  return(
    <motion.div layout initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.97}}
      className={`p-5 rounded-[2rem] bg-[var(--card-bg)] border shadow-sm flex flex-col gap-4 hover:border-blue-500/30 transition-all relative group cursor-pointer ${overdue?"border-red-500/30":nearDeadline?"border-amber-500/30":"border-[var(--border-color)]"}`}
      ...
      <div className="absolute top-4 right-4 z-10" onClick={e=>e.stopPropagation()}>
        ...
        <AnimatePresence>{menuOpen&&(<motion.div initial={{opacity:0,scale:0.95,y:-4}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:-4}} className="absolute right-0 top-8 w-44 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl shadow-xl overflow-hidden z-30">
      ...
      <div className="flex items-center gap-2 flex-wrap pr-8">
        <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase px-2.5 py-1 rounded-full border ${sc.bg} ${sc.color}`}><span className={`w-1.5 h-1.5 rounded-full ${sc.dot}`}/>{sc.label}</span>
        <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-full ${pc.bg} ${pc.color}`}>{pc.label}</span>
      ...
      {/* Tags */}
      {project.tags?.length>0&&<div className="flex flex-wrap gap-1.5">{project.tags.slice(0,3).map(t=><span key={t} className="text-[9px] font-black uppercase tracking-wider bg-slate-500/10 text-slate-500 px-2 py-0.5 rounded-full">{t}</span>)}{project.tags.length>3&&<span className="text-[9px] font-black text-slate-400">+{project.tags.length-3}</span>}</div>}
      ...
      <div className="flex items-center justify-between pt-3 border-t border-[var(--border-color)]" onClick={e=>e.stopPropagation()}>
        ...
        <button onClick={e=>{e.stopPropagation();onAssign(project);}}
          className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-500 transition-all shadow-sm shadow-blue-600/20">
    ...
    <AnimatePresence>
      ...
      <motion.div initial={{x:"100%"}} animate={{x:0}} exit={{x:"100%"}} transition={{type:"spring",damping:28,stiffness:280}}
        ...
        <div className="shrink-0 px-6 py-4 border-b border-[var(--border-color)] bg-[var(--card-bg)]">
          <div className="flex items-start justify-between gap-3 mb-3">
            ...
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={()=>onAssign(project)} className="p-2 rounded-xl hover:bg-blue-500/10 text-slate-400 hover:text-blue-600 transition-colors" title="Assign talent"><UserPlus className="w-4 h-4"/></button>
              <button onClick={()=>onEdit(project)} className="p-2 rounded-xl hover:bg-blue-500/10 text-slate-400 hover:text-blue-600 transition-colors"><Pencil className="w-4 h-4"/></button>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase px-2.5 py-1 rounded-full border ${sc.bg} ${sc.color}`}><span className={`w-1.5 h-1.5 rounded-full ${sc.dot}`}/>{sc.label}</span>
            <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-full ${pc.bg} ${pc.color}`}>{pc.label}</span>
            {project.is_client_facing?<span className="flex items-center gap-1 text-[10px] font-black text-blue-600 bg-blue-500/10 px-2 py-1 rounded-full"><Globe className="w-3 h-3"/>Client-facing</span>:<span className="flex items-center gap-1 text-[10px] font-black text-slate-400 bg-slate-500/10 px-2 py-1 rounded-full"><Lock className="w-3 h-3"/>Internal</span>}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {tab==="overview"&&(<>
            {project.description&&<div className="bg-slate-500/5 rounded-2xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">Description</p><p className="text-sm font-medium text-slate-600 dark:text-slate-300 leading-relaxed">{project.description}</p></div>}
            <div className="bg-slate-500/5 rounded-2xl p-4">
              ...
              <div className="h-2 w-full bg-slate-500/10 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all duration-500 ${localProgress>=100?"bg-emerald-500":localProgress>=60?"bg-blue-500":"bg-amber-500"}`} style={{width:`${localProgress}%`}}/></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {[{label:"Start date",value:fmtDate(project.start_date)},{label:"End date",value:fmtDate(project.end_date)},{label:"Team size",value:`${assignedMembers.length} member${assignedMembers.length!==1?"s":""}`},{label:"Created",value:fmtDate(project.created_at)}].map(m=>(
                <div key={m.label} className="bg-slate-500/5 rounded-2xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{m.label}</p><p className="text-sm font-black dark:text-white">{m.value}</p></div>
              ))}
            </div>
            {project.budget&&<div className="bg-slate-500/5 rounded-2xl p-4">
              ...
              <div className="h-2 w-full bg-slate-500/10 rounded-full overflow-hidden"><div className={`h-full rounded-full ${budgetPct>=90?"bg-red-500":budgetPct>=70?"bg-amber-500":"bg-emerald-500"}`} style={{width:`${budgetPct}%`}}/></div>
            </div>}
            {project.tags?.length>0&&<div className="bg-slate-500/5 rounded-2xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-3">Tags</p><div className="flex flex-wrap gap-2">{project.tags.map(t=><span key={t} className="text-[10px] font-black uppercase tracking-wider bg-blue-500/10 text-blue-600 border border-blue-500/10 px-2 py-0.5 rounded-full">{t}</span>)}</div></div>}
          </>)}
          ...
          {tab==="team"&&(
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={()=>onAssign(project)} className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-500 transition-all shadow-sm shadow-blue-600/20">
                  <UserPlus className="w-3 h-3"/> Assign
                </button>
              </div>
              {assignedMembers.length===0?(
                <div className="py-12 text-center space-y-3 bg-slate-500/5 rounded-[1.5rem] border border-dashed border-[var(--border-color)]"><Users className="w-10 h-10 text-slate-300 mx-auto"/><p className="text-sm font-black dark:text-white">No members assigned yet</p><p className="text-xs text-slate-400">Click "Assign" to add team members</p></div>
              ):assignedMembers.map(m=>(
                <div key={m.id} className="flex items-center gap-3 p-4 rounded-2xl bg-slate-500/5 border border-[var(--border-color)]">
                  <Avatar member={m} size="md"/>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-black dark:text-white truncate">{rName(m)}</p>
                  ...
                  <button onClick={()=>onRemoveMember(m.pmId,rName(m))}
                    className="p-1.5 rounded-lg hover:bg-red-500/10 text-slate-400 hover:text-red-500 transition-colors shrink-0" title="Remove from project">
                    <X className="w-3.5 h-3.5"/>
                  </button>
                </div>
              ))}
          ...
          {tab==="timeline"&&(
            project.start_date&&project.end_date?(
              <div className="space-y-4">
                <div className="bg-slate-500/5 rounded-2xl p-5">
                  ...
                  <div className="h-3 w-full bg-slate-500/10 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all ${localProgress>=100?"bg-emerald-500":"bg-blue-500"}`} style={{width:`${localProgress}%`}}/></div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[{label:"Duration",value:(()=>{const d=Math.ceil((new Date(project.end_date).getTime()-new Date(project.start_date).getTime())/(1000*60*60*24));return`${d} day${d!==1?"s":""}`;})()},{label:daysLeft!==null&&daysLeft<0?"Overdue by":"Days remaining",value:daysLeft!==null?`${Math.abs(daysLeft)} day${Math.abs(daysLeft)!==1?"s":""}` :"—"}].map(m=>(
                    <div key={m.label} className="bg-slate-500/5 rounded-2xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{m.label}</p><p className={`text-sm font-black ${m.label.includes("Overdue")?"text-red-500":"dark:text-white"}`}>{m.value}</p></div>
                  ))}
                </div>
              </div>
            ):(
              <div className="py-12 text-center space-y-3 bg-slate-500/5 rounded-[1.5rem] border border-dashed border-[var(--border-color)]"><Calendar className="w-10 h-10 text-slate-300 mx-auto"/><p className="text-sm font-black dark:text-white">No dates set</p><p className="text-xs text-slate-400">Edit the project to add start and end dates</p></div>
            )
          )}
        </div>
        ...
        <div className="shrink-0 p-4 border-t border-[var(--border-color)] bg-[var(--card-bg)] flex gap-2">
          <button onClick={()=>onAssign(project)} className="flex-1 flex items-center justify-center gap-2 py-3 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-500 transition-all shadow-md shadow-blue-600/20"><UserPlus className="w-3.5 h-3.5"/> Assign talent</button>
    ...
    <div className="w-full space-y-6 pb-20 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center bg-slate-500/5 border border-[var(--border-color)] rounded-xl p-1">
            <button onClick={()=>setViewMode("grid")} className={`p-2 rounded-lg transition-all ${viewMode==="grid"?"bg-[var(--card-bg)] text-blue-600 shadow-sm":"text-slate-400"}`}><LayoutGrid className="w-4 h-4"/></button>
            <button onClick={()=>setViewMode("list")} className={`p-2 rounded-lg transition-all ${viewMode==="list"?"bg-[var(--card-bg)] text-blue-600 shadow-sm":"text-slate-400"}`}><List className="w-4 h-4"/></button>
          </div>
          <button onClick={()=>setEditProject("new")} className="flex items-center gap-2 px-5 py-3 bg-blue-600 text-white font-black text-xs uppercase tracking-widest rounded-xl hover:bg-blue-500 transition-all shadow-md shadow-blue-600/20"><Plus className="w-4 h-4"/> Create project</button>
        </div>
      </div>

      {overdueCount>0&&<motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} className="flex items-center gap-3 p-4 rounded-2xl bg-red-500/5 border border-red-500/20"><AlertTriangle className="w-5 h-5 text-red-500 shrink-0"/><div><p className="text-xs font-black text-red-500 uppercase tracking-widest">{overdueCount} project{overdueCount>1?"s":""} overdue</p><p className="text-[11px] font-medium text-slate-500 mt-0.5">Update progress or extend the end date.</p></div></motion.div>}

      <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400"/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search projects by name, description or tag..." className="w-full pl-10 pr-4 py-3 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/></div>

      <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1">
        {TABS.map(tab=><button key={tab} onClick={()=>setActiveTab(tab)} className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border ${activeTab===tab?"bg-[var(--card-bg)] text-blue-600 border-blue-500/20 shadow-sm":"border-transparent text-slate-400 hover:text-slate-600"}`}>{tab!=="all"&&<span className={`w-1.5 h-1.5 rounded-full ${STATUS_CFG[tab as keyof typeof STATUS_CFG]?.dot??"bg-slate-400"}`}/>}{tab==="all"?"All":STATUS_CFG[tab as keyof typeof STATUS_CFG]?.label??tab}<span className="text-[9px] opacity-60">{counts[tab as keyof typeof counts]??projects.length}</span></button>)}
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[{label:"Total",value:counts.all,color:"text-slate-600 dark:text-white"},{label:"Active",value:counts.active,color:"text-emerald-600"},{label:"Completed",value:counts.completed,color:"text-blue-600"},{label:"Avg progress",value:projects.length>0?`${Math.round(projects.reduce((a,p)=>a+p.progress,0)/projects.length)}%`:"—",color:"text-amber-600"}].map(s=><div key={s.label} className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{s.label}</p><p className={`text-2xl font-black ${s.color}`}>{s.value}</p></div>)}
      </div>
      ...
      <AnimatePresence mode="popLayout">
        {filtered.length>0?(
            ...
            <AnimatePresence>
              {filtered.map(p=>viewMode==="grid"?(
                ...
                <motion.div key={p.id} layout initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.97}}
                  ...
                  className="flex items-center gap-4 p-4 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-color)] hover:border-blue-500/30 transition-all cursor-pointer group">
                  <div className="flex-1 min-w-0">
          ...
          <motion.div initial={{opacity:0}} animate={{opacity:1}} className="py-24 text-center space-y-4 bg-slate-500/5 rounded-[2.5rem] border border-dashed border-[var(--border-color)]">
            ...
            {!search&&<button onClick={()=>setEditProject("new")} className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-500 transition-all shadow-md shadow-blue-600/20"><Plus className="w-3.5 h-3.5"/> Create project</button>}
          </motion.div>
        )}
      </AnimatePresence>
