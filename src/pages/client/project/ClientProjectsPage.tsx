...
// ─── Config ───────────────────────────────────────────────────────────────────
const STATUS_CFG = {
  active:    {label:"Active",    color:"text-slate-900",bg:"bg-slate-100 border-slate-200",dot:"bg-[#1A1C21]"},
  on_hold:   {label:"On hold",   color:"text-slate-900",  bg:"bg-slate-100 border-slate-200",    dot:"bg-[#1A1C21]"  },
  completed: {label:"Completed", color:"text-[#1A1C21]",   bg:"bg-slate-100 border-slate-200",      dot:"bg-[#1A1C21]"   },
...
const PRIORITY_CFG = {
  ...
  medium:   {label:"Medium",   color:"text-[#1A1C21]",  bg:"bg-slate-100"  },
  high:     {label:"High",     color:"text-slate-900", bg:"bg-slate-100" },
  critical: {label:"Critical", color:"text-red-500",   bg:"bg-red-500/10"   },
};
const ONLINE_DOT: Record<string,string> = {
  online:"bg-[#1A1C21]", away:"bg-[#1A1C21]", offline:"bg-slate-400",
};
...
function createId(){return crypto.randomUUID?.() ?? Math.random().toString(36).slice(2,11);}
...
function Avatar({member,size="sm"}:{member:WorkforceMember;size?:"sm"|"md"|"lg"}){
  ...
  const sz=size==="lg"?"w-12 h-12 rounded-2xl":size==="md"?"w-10 h-10 rounded-xl":"w-8 h-8 rounded-lg";
  return(
    <div className="relative shrink-0">
      ...
      <span className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-[var(--card-bg)] ${dot}`}/>
    </div>
  );
}
...
function ProgressBar({value,showLabel=true}:{value:number;showLabel?:boolean}){
  const color=value>=100?"bg-[#1A1C21]":value>=60?"bg-[#1A1C21]":value>=30?"bg-[#1A1C21]":"bg-slate-400";
  return(
    <div className="space-y-1">
      ...
      <div className="h-1.5 w-full bg-slate-500/10 rounded-full overflow-hidden"><div className={`h-full ${color} rounded-full transition-all duration-500`} style={{width:`${value}%`}}/></div>
    </div>
  );
}
    ...
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onCancel}
        ...
        <motion.div initial={{opacity:0,scale:0.95,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:16}}
          ...
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${danger?"bg-red-500/10":"bg-slate-100"}`}>
            <Trash2 className={`w-5 h-5 ${danger?"text-red-500":"text-slate-900"}`}/>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={onConfirm} className={`flex-1 py-3 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${danger?"bg-red-500 hover:bg-red-400":"bg-[#1A1C21] hover:bg-amber-400"}`}>{confirmLabel}</button>
          </div>
        </motion.div>
      </motion.div>
    </AnimatePresence>
...
function TagInput({tags,onAdd,onRemove}:{tags:string[];onAdd:(v:string)=>void;onRemove:(v:string)=>void;}){
  ...
  return(
    <div className="min-h-[44px] bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-3 py-2 flex flex-wrap gap-2 items-center focus-within:ring-2 ring-slate-900/10 transition-all cursor-text"
      onClick={e=>(e.currentTarget.querySelector("input") as HTMLInputElement)?.focus()}>
      {tags.map(t=><span key={t} className="inline-flex items-center gap-1 text-[11px] font-black uppercase tracking-wider bg-slate-100 text-[#1A1C21] px-2 py-0.5 rounded-lg">{t}<button type="button" onClick={()=>onRemove(t)} className="hover:text-red-500 transition-colors"><X className="w-2.5 h-2.5"/></button></span>)}
...
function AssignTalentModal({project,allMembers,currentProjectMembers,onAssign,onRemove,onClose,saving}:{
  ...
  const AVAIL_BADGE:Record<string,string>={
    available:"bg-slate-100 text-slate-900 border-slate-200",
    on_leave:"bg-slate-100 text-slate-900 border-slate-200",
    ...
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose}
        ...
        <motion.div initial={{opacity:0,scale:0.95,y:20}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95}}
          onClick={e=>e.stopPropagation()}
          className="w-full max-w-md bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="px-7 pt-7 pb-4">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-xs font-bold text-[#1A1C21] mt-0.5 truncate">{project.name}</p>
              </div>
              <button onClick={onClose} className="p-2 rounded-xl hover:bg-slate-500/10 text-slate-400 transition-colors flex-shrink-0"><X className="w-4 h-4"/></button>
            </div>
            {/* Search */}
            <div className="relative">
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)}
                ...
                className="w-full pl-9 pr-4 py-2.5 bg-slate-500/5 border border-[var(--border-color)] rounded-xl text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/>
            </div>
          </div>
            ...
            {filtered.length===0?(
              ...
              return(
                <div key={m.id}
                  ...
                  className={`flex items-center gap-3 px-3 py-3.5 transition-all ${isAssigned?"opacity-70":"cursor-pointer hover:bg-slate-500/5"} ${isSelected?"bg-slate-50":""}`}>
                  <Avatar member={m} size="md"/>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-black dark:text-white truncate">{rName(m)}</p>
                      {availBadge&&avail!=="available"&&(
                        <span className={`text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-lg border ${availBadge}`}>
                          {avail.replace("_"," ")}
                        </span>
                      )}
                    </div>
                    ...
                    {isAssigned&&pm?.role_on_project&&(
                      <p className="text-[9px] font-black text-[#1A1C21] uppercase tracking-wider mt-0.5">
                  ...
                  <div className="shrink-0">
                    {isAssigned?(
                      ...
                      <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${isSelected?"bg-[#1A1C21] border-[#1A1C21]":"border-slate-300 dark:border-slate-600"}`}>
            ...
            {selected.size>0&&(
              <div>
                ...
                <input value={roleInput} onChange={e=>setRoleInput(e.target.value)}
                  ...
                  className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-2.5 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/>
              </div>
            )}
            <button onClick={handleAssign} disabled={selected.size===0||saving}
              className="w-full py-3.5 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all flex items-center justify-center gap-2 shadow-md shadow-slate-900/10 disabled:opacity-40 disabled:cursor-not-allowed">
  ...
  return(
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose}
        ...
        <motion.div initial={{opacity:0,scale:0.95,y:20}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95}}
          onClick={e=>e.stopPropagation()}
          className="w-full max-w-xl bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="px-8 pb-0 space-y-4 max-h-[60vh] overflow-y-auto">
            <div>
              ...
              <input value={form.name} onChange={e=>upd("name",e.target.value)} placeholder="e.g. Flowboard Mobile App"
                className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/>
            </div>
            <div>
              ...
              <textarea value={form.description} onChange={e=>upd("description",e.target.value)} rows={3} placeholder="What is this project about?"
                className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl p-4 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all resize-none"/>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Status</label>
                <div className="relative">
                  <select value={form.status} onChange={e=>upd("status",e.target.value)} className="w-full appearance-none bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 cursor-pointer">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Start date</label><input type="date" value={form.start_date} onChange={e=>upd("start_date",e.target.value)} className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/></div>
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">End date</label><input type="date" value={form.end_date} onChange={e=>upd("end_date",e.target.value)} className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2"><DollarSign className="inline w-3 h-3 mr-0.5"/>Total budget</label><input value={form.budget} onChange={e=>upd("budget",e.target.value)} type="number" placeholder="e.g. 50000" className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/></div>
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Budget spent</label><input value={form.budget_spent} onChange={e=>upd("budget_spent",e.target.value)} type="number" placeholder="e.g. 12000" className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/></div>
            </div>
            <div>
              ...
              <input type="range" min={0} max={100} step={5} value={form.progress} onChange={e=>upd("progress",Number(e.target.value))} className="w-full accent-black cursor-pointer"/>
            </div>
            <div>
            ...
            <div className="flex items-center justify-between p-4 rounded-xl bg-slate-500/5 border border-[var(--border-color)]">
              <div className="flex items-center gap-3">
                {form.is_client_facing?<Globe className="w-4 h-4 text-[#1A1C21]"/>:<Lock className="w-4 h-4 text-slate-400"/>}
              ...
              <button type="button" onClick={()=>upd("is_client_facing",!form.is_client_facing)}
                className={`w-12 h-6 rounded-full transition-all relative ${form.is_client_facing?"bg-[#1A1C21]":"bg-slate-300 dark:bg-slate-600"}`}>
                <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-all ${form.is_client_facing?"left-6":"left-0.5"}`}/>
              </button>
            </div>
          </div>
          <div className="px-8 py-6 flex gap-3">
            ...
            <button onClick={()=>onSave(form)} disabled={!form.name.trim()||saving} className="flex-1 py-3.5 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all flex items-center justify-center gap-2 shadow-md shadow-slate-900/10 disabled:opacity-40">
...
function ProjectCard({project,projectMembers,allMembers,onView,onEdit,onDelete,onAssign}:{
  ...
  return(
    <motion.div layout initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.97}}
      className={`p-5 rounded-2xl bg-[var(--card-bg)] border shadow-sm flex flex-col gap-4 hover:border-[#1A1C21]/30 transition-all relative group cursor-pointer ${overdue?"border-red-500/30":nearDeadline?"border-amber-500/30":"border-[var(--border-color)]"}`}
      ...
      <div className="absolute top-4 right-4 z-10" onClick={e=>e.stopPropagation()}>
        ...
        <AnimatePresence>{menuOpen&&(<motion.div initial={{opacity:0,scale:0.95,y:-4}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:-4}} className="absolute right-0 top-8 w-44 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl shadow-xl overflow-hidden z-30">
          ...
          <button onClick={()=>{onAssign(project);setMenuOpen(false);}} className="flex items-center gap-2 w-full px-4 py-2.5 text-xs font-bold text-[#1A1C21] hover:bg-slate-50 transition-colors"><UserPlus className="w-3.5 h-3.5"/> Assign talent</button>
      ...
      <div className="flex items-center gap-2 flex-wrap pr-8">
        <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase px-2.5 py-1 rounded-lg border ${sc.bg} ${sc.color}`}><span className={`w-1.5 h-1.5 rounded-full ${sc.dot}`}/>{sc.label}</span>
        <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-lg ${pc.bg} ${pc.color}`}>{pc.label}</span>
        {project.is_client_facing&&<Globe className="w-3.5 h-3.5 text-[#1A1C21]"/>}
      </div>
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1.5">
        {project.end_date&&<span className={`flex items-center gap-1 text-[11px] font-bold ${overdue?"text-red-500":nearDeadline?"text-slate-900":"text-slate-400"}`}><Calendar className="w-3 h-3"/>{overdue?`Overdue ${Math.abs(daysLeft!)}d`:nearDeadline?`Due in ${daysLeft}d`:fmtDate(project.end_date)}</span>}
      ...
      {/* Tags */}
      {project.tags?.length>0&&<div className="flex flex-wrap gap-1.5">{project.tags.slice(0,3).map(t=><span key={t} className="text-[9px] font-black uppercase tracking-wider bg-slate-500/10 text-slate-500 px-2 py-0.5 rounded-lg">{t}</span>)}{project.tags.length>3&&<span className="text-[9px] font-black text-slate-400">+{project.tags.length-3}</span>}</div>}
      ...
      <div className="flex items-center justify-between pt-3 border-t border-[var(--border-color)]" onClick={e=>e.stopPropagation()}>
        <div className="flex -space-x-2">
          ...
          {assignedMembers.length>5&&<div className="w-7 h-7 rounded-full bg-slate-500/20 border-2 border-[var(--card-bg)] flex items-center justify-center"><span className="text-[8px] font-black text-slate-500">+{assignedMembers.length-5}</span></div>}
        </div>
        <button onClick={e=>{e.stopPropagation();onAssign(project);}}
          className="flex items-center gap-1.5 px-3 py-2 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all shadow-sm shadow-slate-900/10">
...
// ─── Project Drawer ───────────────────────────────────────────────────────────
function ProjectDrawer({project,projectMembers,allMembers,projectBoards,projectCards,boardInputs,cardInputs,onBoardInputChange,onCreateBoard,onCardInputChange,onCreateCard,onClose,onEdit,onDelete,onAssign,onRemoveMember,onProgressUpdate}:{
  project:Project; projectMembers:ProjectMember[]; allMembers:WorkforceMember[];
  projectBoards: Record<string,Board[]>;
  projectCards: Record<string,Card[]>;
  boardInputs: Record<string,{name:string;department:string}>;
  cardInputs: Record<string,string>;
  onBoardInputChange:(projectId:string,field:"name"|"department",value:string)=>void;
  onCreateBoard:(projectId:string)=>void;
  onCardInputChange:(boardId:string,value:string)=>void;
  onCreateCard:(projectId:string,boardId:string)=>void;
  ...
  const [tab,setTab]=useState<"overview"|"team"|"timeline"|"boards">("overview");
    ...
    <AnimatePresence>
      ...
      <motion.div initial={{x:"100%"}} animate={{x:0}} exit={{x:"100%"}} transition={{type:"spring",damping:28,stiffness:280}}
        ...
        <div className="shrink-0 px-6 py-4 border-b border-[var(--border-color)] bg-[var(--card-bg)]">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-black uppercase tracking-widest text-[#1A1C21] mb-0.5">Project details</p>
              <h2 className="text-lg font-black dark:text-white tracking-tight leading-tight truncate">{project.name}</h2>
            </div>
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={()=>onAssign(project)} className="p-2 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-[#1A1C21] transition-colors" title="Assign talent"><UserPlus className="w-4 h-4"/></button>
              <button onClick={()=>onEdit(project)} className="p-2 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-[#1A1C21] transition-colors"><Pencil className="w-4 h-4"/></button>
              <button onClick={()=>setConfirmDelete(true)} className="p-2 rounded-xl hover:bg-red-500/10 text-slate-400 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4"/></button>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase px-2.5 py-1 rounded-lg border ${sc.bg} ${sc.color}`}><span className={`w-1.5 h-1.5 rounded-full ${sc.dot}`}/>{sc.label}</span>
            <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-lg ${pc.bg} ${pc.color}`}>{pc.label}</span>
            {project.is_client_facing?<span className="flex items-center gap-1 text-[10px] font-black text-[#1A1C21] bg-slate-100 px-2 py-1 rounded-lg"><Globe className="w-3 h-3"/>Client-facing</span>:<span className="flex items-center gap-1 text-[10px] font-black text-slate-400 bg-slate-500/10 px-2 py-1 rounded-lg"><Lock className="w-3 h-3"/>Internal</span>}
          </div>
        </div>

        {/* Deadline warning */}
        {(overdue||nearDeadline)&&<div className={`shrink-0 flex items-center gap-2 px-6 py-3 border-b ${overdue?"bg-red-500/5 border-red-500/20":"bg-[#1A1C21]/5 border-slate-200"}`}><AlertTriangle className={`w-4 h-4 shrink-0 ${overdue?"text-red-500":"text-slate-900"}`}/><p className={`text-[11px] font-black ${overdue?"text-red-500":"text-slate-900"}`}>{overdue?`Overdue by ${Math.abs(daysLeft!)} day${Math.abs(daysLeft!)!==1?"s":""}` :`Deadline in ${daysLeft} day${daysLeft!==1?"s":""}`}</p></div>}

        {/* Tabs */}
        <div className="shrink-0 flex gap-1 px-6 py-3 border-b border-[var(--border-color)]">
          {[{key:"overview",label:"Overview",icon:BarChart3},{key:"team",label:"Team",icon:Users},{key:"timeline",label:"Timeline",icon:Calendar},{key:"boards",label:"Boards",icon:FolderKanban}].map(t=>(
            <button key={t.key} onClick={()=>setTab(t.key as any)} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border ${tab===t.key?"bg-[var(--card-bg)] text-[#1A1C21] border-slate-200 shadow-sm":"border-transparent text-slate-400 hover:text-slate-600"}`}><t.icon className="w-3.5 h-3.5"/>{t.label}</button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {tab==="overview"&&(<>
            {project.description&&<div className="bg-slate-500/5 rounded-xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">Description</p><p className="text-sm font-medium text-slate-600 dark:text-slate-300 leading-relaxed">{project.description}</p></div>}
            <div className="bg-slate-500/5 rounded-xl p-4">
              ...
              <input type="range" min={0} max={100} step={5} value={localProgress} onChange={e=>setLocalProgress(Number(e.target.value))} onMouseUp={()=>onProgressUpdate(project.id,localProgress)} onTouchEnd={()=>onProgressUpdate(project.id,localProgress)} className="w-full accent-black cursor-pointer mb-2"/>
              <div className="h-2 w-full bg-slate-500/10 rounded-xl overflow-hidden"><div className={`h-full rounded-xl transition-all duration-500 ${localProgress>=100?"bg-[#1A1C21]":localProgress>=60?"bg-[#1A1C21]":"bg-[#1A1C21]"}`} style={{width:`${localProgress}%`}}/></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {[{label:"Start date",value:fmtDate(project.start_date)},{label:"End date",value:fmtDate(project.end_date)},{label:"Team size",value:`${assignedMembers.length} member${assignedMembers.length!==1?"s":""}`},{label:"Created",value:fmtDate(project.created_at)}].map(m=>(
                <div key={m.label} className="bg-slate-500/5 rounded-xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{m.label}</p><p className="text-sm font-black dark:text-white">{m.value}</p></div>
              ))}
            </div>
            {project.budget&&<div className="bg-slate-500/5 rounded-xl p-4">
              <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-3">Budget</p>
              <div className="flex items-end justify-between mb-2"><div><p className="text-xl font-black dark:text-white">{fmtCurrency(project.budget_spent,project.budget_currency)}</p><p className="text-[10px] font-bold text-slate-400">of {fmtCurrency(project.budget,project.budget_currency)}</p></div><span className={`text-sm font-black ${budgetPct>=90?"text-red-500":budgetPct>=70?"text-slate-900":"text-slate-900"}`}>{budgetPct}%</span></div>
              <div className="h-2 w-full bg-slate-500/10 rounded-xl overflow-hidden"><div className={`h-full rounded-xl ${budgetPct>=90?"bg-red-500":budgetPct>=70?"bg-[#1A1C21]":"bg-[#1A1C21]"}`} style={{width:`${budgetPct}%`}}/></div>
            </div>}
            {project.tags?.length>0&&<div className="bg-slate-500/5 rounded-xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-3">Tags</p><div className="flex flex-wrap gap-2">{project.tags.map(t=><span key={t} className="text-[10px] font-black uppercase tracking-wider bg-slate-100 text-[#1A1C21] border border-slate-200 px-2 py-0.5 rounded-xl">{t}</span>)}</div></div>}
          </>)}
          ...
          {tab==="team"&&(
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={()=>onAssign(project)} className="flex items-center gap-1.5 px-3 py-2 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all shadow-sm shadow-slate-900/10">
                  <UserPlus className="w-3 h-3"/> Assign
                </button>
              </div>
              {assignedMembers.length===0?(
                <div className="py-12 text-center space-y-3 bg-slate-500/5 rounded-2xl border border-dashed border-[var(--border-color)]"><Users className="w-10 h-10 text-slate-300 mx-auto"/><p className="text-sm font-black dark:text-white">No members assigned yet</p><p className="text-xs text-slate-400">Click "Assign" to add team members</p></div>
              ):assignedMembers.map(m=>(
                <div key={m.id} className="flex items-center gap-3 p-4 rounded-xl bg-slate-500/5 border border-[var(--border-color)]">
                  <Avatar member={m} size="md"/>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-black dark:text-white truncate">{rName(m)}</p>
                  ...
                  <button onClick={()=>onRemoveMember(m.pmId,rName(m))}
                    className="p-1.5 rounded-xl hover:bg-red-500/10 text-slate-400 hover:text-red-500 transition-colors shrink-0" title="Remove from project">
                    <X className="w-3.5 h-3.5"/>
                  </button>
                </div>
              ))}
          ...
          {tab==="timeline"&&(
            project.start_date&&project.end_date?(
              <div className="space-y-4">
                <div className="bg-slate-500/5 rounded-xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="text-center"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">End</p><p className={`text-sm font-black ${overdue?"text-red-500":nearDeadline?"text-slate-900":"dark:text-white"}`}>{fmtDate(project.end_date)}</p></div>
                  </div>
                  <div className="h-3 w-full bg-slate-500/10 rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all ${localProgress>=100?"bg-[#1A1C21]":"bg-[#1A1C21]"}`} style={{width:`${localProgress}%`}}/></div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[{label:"Duration",value:(()=>{const d=Math.ceil((new Date(project.end_date).getTime()-new Date(project.start_date).getTime())/(1000*60*60*24));return`${d} day${d!==1?"s":""}`;})()},{label:daysLeft!==null&&daysLeft<0?"Overdue by":"Days remaining",value:daysLeft!==null?`${Math.abs(daysLeft)} day${Math.abs(daysLeft)!==1?"s":""}` :"—"}].map(m=>(
                    <div key={m.label} className="bg-slate-500/5 rounded-xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{m.label}</p><p className={`text-sm font-black ${m.label.includes("Overdue")?"text-red-500":"dark:text-white"}`}>{m.value}</p></div>
                  ))}
                </div>
              </div>
            ):(
              <div className="py-12 text-center space-y-3 bg-slate-500/5 rounded-xl border border-dashed border-[var(--border-color)]"><Calendar className="w-10 h-10 text-slate-300 mx-auto"/><p className="text-sm font-black dark:text-white">No dates set</p><p className="text-xs text-slate-400">Edit the project to add start and end dates</p></div>
            )
          )}

          {tab==="boards"&&(
            <div className="space-y-6">
              <div className="grid gap-3 md:grid-cols-[1.1fr_auto]">
                <div className="grid gap-3">
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Board name</label>
                    <input value={boardInputs[project.id]?.name ?? ""} onChange={e=>onBoardInputChange(project.id,"name",e.target.value)} placeholder="e.g. Product launch"
                      className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-2xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/>
                  </div>
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Department</label>
                    <input value={boardInputs[project.id]?.department ?? ""} onChange={e=>onBoardInputChange(project.id,"department",e.target.value)} placeholder="e.g. Engineering"
                      className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-2xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/>
                  </div>
                </div>
                <button onClick={()=>onCreateBoard(project.id)} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#1A1C21] px-4 py-3 text-sm font-black text-white hover:bg-black transition">Create board</button>
              </div>
              {(() => {
                const boards = projectBoards[project.id] ?? [];
                return boards.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-[var(--border-color)] bg-slate-50 p-8 text-center">
                    <FolderKanban className="w-10 h-10 text-slate-300 mx-auto" />
                    <p className="text-base font-black text-slate-950 mt-4">No boards yet</p>
                    <p className="mt-2 text-sm text-slate-500">Create a board to organize work for this project.</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {boards.map(board=>{
                      const cards = projectCards[board.id] ?? [];
                    return (
                      <div key={board.id} className="rounded-2xl border border-[var(--border-color)] bg-slate-50 p-4">
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                          <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{board.department}</p>
                            <h3 className="text-base font-black text-slate-950 mt-1">{board.name}</h3>
                          </div>
                          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">{cards.length} card{cards.length !== 1 ? "s" : ""}</span>
                        </div>
                        <p className="mt-3 text-sm text-slate-500">{board.description || "No description added."}</p>
                        ...
                        <div className="mt-4 space-y-3">
                              ...
                              {cards.map(card => (
                                <div key={card.id} className="rounded-2xl border border-slate-200 bg-white p-3">
                                  <div className="flex items-center justify-between gap-3">
                                    <p className="text-sm font-black text-slate-950 truncate">{card.title}</p>
                                    <span className="text-[10px] uppercase tracking-[0.2em] text-slate-400">{card.labels.length} label{card.labels.length!==1?"s":""}</span>
                                  </div>
                                  <div className="mt-2 text-[10px] text-slate-500 grid gap-1 sm:grid-cols-2">
                                    <span>Start: {card.start_date ?? "—"}</span>
                          ...
                          <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
                            <input
                              value={cardInputs[board.id] ?? ""}
                              onChange={e=>onCardInputChange(board.id,e.target.value)}
                              placeholder="New card title"
                              className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900"
                            />
                            <button
                              onClick={()=>onCreateCard(project.id,board.id)}
                              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#1A1C21] px-4 py-3 text-sm font-black text-white hover:bg-black transition"
                            >Add card</button>
                          </div>
                        </div>
                      </div>
        ...
        <div className="shrink-0 p-4 border-t border-[var(--border-color)] bg-[var(--card-bg)] flex gap-2">
          <button onClick={()=>onAssign(project)} className="flex-1 flex items-center justify-center gap-2 py-3 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all shadow-md shadow-slate-900/10"><UserPlus className="w-3.5 h-3.5"/> Assign talent</button>
          <button onClick={()=>onEdit(project)} className="px-4 py-3 bg-slate-500/10 text-slate-500 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-500/20 transition-all flex items-center gap-2"><Pencil className="w-3.5 h-3.5"/>Edit</button>
        </div>
      </motion.div>
...
export default function ClientProjectsPage(){
  ...
  const [projectCards,setProjectCards]=useState<Record<string,Card[]>>({});
  const [boardInputs,setBoardInputs]=useState<Record<string,{name:string;department:string}>>({});
  const [cardInputs,setCardInputs]=useState<Record<string,string>>({});

  const handleBoardInputChange=(projectId:string, field:"name"|"department", value:string)=>{
    setBoardInputs(prev=>({
      ...prev,
      [projectId]: {
        name: field==="name" ? value : prev[projectId]?.name ?? "",
        department: field==="department" ? value : prev[projectId]?.department ?? "",
      },
    }));
  };
  ...
  const handleCreateCard=async(projectId:string, boardId:string)=>{
    const title = cardInputs[boardId]?.trim() ?? "";
    if(!title) return;
    setSaving(true);
    try{
      const user = await getSupabaseUser();
      const {data,error} = await supabase.from("project_cards").insert({
      ...
      if(error) {
        if (isMissingTableError(error)) {
          toast({variant:"destructive",title:"Boards unavailable",description:"The project boards tables have not been created in Supabase yet."});
          return;
        }
        throw error;
      }
  ...
  const loadBoardsAndCards=async(projectIds:string[])=>{
    ...
    const [{data:boards,error:boardsErr},{data:cards,error:cardsErr}] = await Promise.all([
      supabase.from("project_boards").select("*").in("project_id",projectIds),
      supabase.from("project_cards").select("*").in("project_id",projectIds),
    ]);

    if(boardsErr||cardsErr){
      if (isMissingTableError(boardsErr) || isMissingTableError(cardsErr)) {
        setProjectBoards({});
        setProjectCards({});
        return;
      }
      throw boardsErr||cardsErr;
    }

    const boardMap = (boards||[]).reduce<Record<string,Board[]>>((acc,board:any)=>{
      ...
      acc[normalized.project_id] = [...(acc[normalized.project_id] ?? []), normalized];
      return acc;
    }, {});

    const cardMap = (cards||[]).reduce<Record<string,Card[]>>((acc,card:any)=>{
    ...
    setProjectBoards(boardMap);
    setProjectCards(cardMap);
  };

  const handleCreateBoard=async(projectId:string)=>{
    const input = boardInputs[projectId] ?? { name: "", department: "" };
  ...
  const fetchAll=useCallback(async()=>{
    ...
    try{
      ...
      await loadBoardsAndCards((projData??[]).map((p:any)=>p.id));
  ...
  if(loading) return <div className="w-full flex items-center justify-center py-32"><div className="flex flex-col items-center gap-4"><Loader2 className="w-8 h-8 text-[#1A1C21] animate-spin"/><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Loading projects...</p></div></div>;
  if(error) return <div className="w-full flex flex-col items-center justify-center py-32 gap-4"><p className="text-sm font-bold text-red-500">{error}</p><button onClick={fetchAll} className="flex items-center gap-2 px-5 py-3 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl"><RefreshCw className="w-3.5 h-3.5"/> Retry</button></div>;
  ...
  return(
    <div className="w-full space-y-6 pb-20 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center bg-slate-500/5 border border-[var(--border-color)] rounded-xl p-1">
            <button onClick={()=>setViewMode("grid")} className={`p-2 rounded-lg transition-all ${viewMode==="grid"?"bg-[var(--card-bg)] text-[#1A1C21] shadow-sm":"text-slate-400"}`}><LayoutGrid className="w-4 h-4"/></button>
            <button onClick={()=>setViewMode("list")} className={`p-2 rounded-lg transition-all ${viewMode==="list"?"bg-[var(--card-bg)] text-[#1A1C21] shadow-sm":"text-slate-400"}`}><List className="w-4 h-4"/></button>
          </div>
          <button onClick={()=>setEditProject("new")} className="flex items-center gap-2 px-5 py-3 bg-[#1A1C21] text-white font-bold text-xs tracking-tight rounded-xl hover:bg-black transition-all shadow-md shadow-slate-900/10"><Plus className="w-4 h-4"/> Create project</button>
        </div>
      </div>

      {overdueCount>0&&<motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} className="flex items-center gap-3 p-4 rounded-xl bg-red-500/5 border border-red-500/20"><AlertTriangle className="w-5 h-5 text-red-500 shrink-0"/><div><p className="text-xs font-black text-red-500 uppercase tracking-widest">{overdueCount} project{overdueCount>1?"s":""} overdue</p><p className="text-[11px] font-medium text-slate-500 mt-0.5">Update progress or extend the end date.</p></div></motion.div>}

      <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400"/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search projects by name, description or tag..." className="w-full pl-10 pr-4 py-3 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/></div>

      <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1">
        {TABS.map(tab=><button key={tab} onClick={()=>setActiveTab(tab)} className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl text-[11px] font-bold tracking-tight transition-all border ${activeTab===tab?"bg-[var(--card-bg)] text-[#1A1C21] border-slate-200 shadow-sm":"border-transparent text-slate-400 hover:text-slate-600"}`}>{tab!=="all"&&<span className={`w-1.5 h-1.5 rounded-full ${STATUS_CFG[tab as keyof typeof STATUS_CFG]?.dot??"bg-slate-400"}`}/>}{tab==="all"?"All":STATUS_CFG[tab as keyof typeof STATUS_CFG]?.label??tab}<span className="text-[9px] opacity-60">{counts[tab as keyof typeof counts]??projects.length}</span></button>)}
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[{label:"Total",value:counts.all,color:"text-slate-600 dark:text-white"},{label:"Active",value:counts.active,color:"text-slate-900"},{label:"Completed",value:counts.completed,color:"text-[#1A1C21]"},{label:"Avg progress",value:projects.length>0?`${Math.round(projects.reduce((a,p)=>a+p.progress,0)/projects.length)}%`:"—",color:"text-slate-900"}].map(s=><div key={s.label} className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{s.label}</p><p className={`text-2xl font-black ${s.color}`}>{s.value}</p></div>)}
      </div>
      ...
      <AnimatePresence mode="popLayout">
        {filtered.length>0?(
            ...
            <AnimatePresence>
              {filtered.map(p=>viewMode==="grid"?(
                ...
                <motion.div key={p.id} layout initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.97}}
                  ...
                  className="flex items-center gap-4 p-4 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-color)] hover:border-[#1A1C21]/30 transition-all cursor-pointer group">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1"><span className={`w-2 h-2 rounded-full shrink-0 ${STATUS_CFG[p.status].dot}`}/><h3 className="text-sm font-black dark:text-white truncate">{p.name}</h3></div>
          ...
          <motion.div initial={{opacity:0}} animate={{opacity:1}} className="py-24 text-center space-y-4 bg-slate-500/5 rounded-2xl border border-dashed border-[var(--border-color)]">
            ...
            {!search&&<button onClick={()=>setEditProject("new")} className="inline-flex items-center gap-2 px-6 py-3 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all shadow-md shadow-slate-900/10"><Plus className="w-3.5 h-3.5"/> Create project</button>}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* Modals */}
      <AnimatePresence>
        {viewProject&&<ProjectDrawer project={viewProject} projectMembers={getPM(viewProject.id)} allMembers={members} projectBoards={projectBoards} projectCards={projectCards} boardInputs={boardInputs} cardInputs={cardInputs} onBoardInputChange={handleBoardInputChange} onCreateBoard={handleCreateBoard} onCardInputChange={handleCardInputChange} onCreateCard={handleCreateCard} onClose={()=>setViewProject(null)} onEdit={p=>{setViewProject(null);setEditProject(p);}} onDelete={handleDelete} onAssign={p=>{setViewProject(null);setAssignProject(p);}} onRemoveMember={handleRemoveMember} onProgressUpdate={handleProgressUpdate}/>}
      </AnimatePresence>
      <AnimatePresence>
