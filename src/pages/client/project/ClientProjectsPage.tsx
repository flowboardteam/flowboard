...
function TagInput({tags,onAdd,onRemove}:{tags:string[];onAdd:(v:string)=>void;onRemove:(v:string)=>void;}){
  ...
  return(
    <div className="min-h-[44px] bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-3 py-2 flex flex-wrap gap-2 items-center focus-within:ring-2 ring-blue-500/20 transition-all cursor-text"
      ...
      {tags.map(t=><span key={t} className="inline-flex items-center gap-1 text-[11px] font-black uppercase tracking-wider bg-blue-500/10 text-blue-600 px-2 py-0.5 rounded-lg">{t}<button type="button" onClick={()=>onRemove(t)} className="hover:text-red-500 transition-colors"><X className="w-2.5 h-2.5"/></button></span>)}
                ...
                <div key={m.id}
                  ...
                  <div className="shrink-0">
                    {isAssigned?(
                      ...
                      <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${isSelected?"bg-blue-600 border-blue-600":"border-slate-300 dark:border-slate-600"}`}>
...
function ProjectCard({project,projectMembers,allMembers,onView,onEdit,onDelete,onAssign}:{
  ...
  return(
    <motion.div layout initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.97}}
      className={`p-5 rounded-2xl bg-[var(--card-bg)] border shadow-sm flex flex-col gap-4 hover:border-blue-500/30 transition-all relative group cursor-pointer ${overdue?"border-red-500/30":nearDeadline?"border-amber-500/30":"border-[var(--border-color)]"}`}
      ...
      <motion.div initial={{x:"100%"}} animate={{x:0}} exit={{x:"100%"}} transition={{type:"spring",damping:28,stiffness:280}}
        ...
        <div className="shrink-0 px-6 py-4 border-b border-[var(--border-color)] bg-[var(--card-bg)]">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {project.is_client_facing?<span className="flex items-center gap-1 text-[10px] font-black text-blue-600 bg-blue-500/10 px-2 py-1 rounded-lg"><Globe className="w-3 h-3"/>Client-facing</span>:<span className="flex items-center gap-1 text-[10px] font-black text-slate-400 bg-slate-500/10 px-2 py-1 rounded-lg"><Lock className="w-3 h-3"/>Internal</span>}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {tab==="overview"&&(<>
            ...
            <div className="bg-slate-500/5 rounded-xl p-4">
              ...
              <div className="h-2 w-full bg-slate-500/10 rounded-xl overflow-hidden"><div className={`h-full rounded-xl transition-all duration-500 ${localProgress>=100?"bg-emerald-500":localProgress>=60?"bg-blue-500":"bg-amber-500"}`} style={{width:`${localProgress}%`}}/></div>
            </div>
            ...
            {project.budget&&<div className="bg-slate-500/5 rounded-xl p-4">
              ...
              <div className="h-2 w-full bg-slate-500/10 rounded-xl overflow-hidden"><div className={`h-full rounded-xl ${budgetPct>=90?"bg-red-500":budgetPct>=70?"bg-amber-500":"bg-emerald-500"}`} style={{width:`${budgetPct}%`}}/></div>
            </div>}
            {project.tags?.length>0&&<div className="bg-slate-500/5 rounded-xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-3">Tags</p><div className="flex flex-wrap gap-2">{project.tags.map(t=><span key={t} className="text-[10px] font-black uppercase tracking-wider bg-blue-500/10 text-blue-600 border border-blue-500/10 px-2 py-0.5 rounded-xl">{t}</span>)}</div></div>}
          </>)}
    ...
    <div className="w-full space-y-6 pb-20 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold dark:text-white tracking-tight">Your projects.</h1>
