...
function MasterCandidateCard({ candidate, onStageChange, onRemove, onHire }) {
  ...
  return (
    <motion.div
      ...
      className="p-6 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-color)] flex flex-col gap-4 hover:border-blue-500/30 transition-all relative shadow-sm"
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11px] font-black text-blue-600 uppercase tracking-widest truncate mb-0.5">{candidate.role_title}</p>
      ...
      <div className="flex gap-2 pt-4 mt-auto border-t border-[var(--border-color)]">
        ...
        {candidate.status === "hired" ? (
          ...
          <button onClick={() => onHire(candidate)}
            className="flex items-center gap-1.5 px-5 py-2.5 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-500 transition-all shadow-md shadow-blue-600/20">
    ...
    return (
      <div className="max-w-7xl mx-auto pb-20 flex items-center justify-center py-32">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Loading Master Shortlist Hub...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
        <div className="space-y-3">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold dark:text-white tracking-tight">
            Ultimate <span className="text-blue-600">Candidate Pipeline.</span>
          </h1>
      ...
      <div className="p-5 rounded-2xl bg-[var(--card-bg)] border border-[var(--border-color)] shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-slate-500/5 border border-[var(--border-color)] rounded-xl text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"
            />
          </div>
          <div className="relative min-w-[240px]">
            <select value={activeRoleFilter} onChange={e => setActiveRoleFilter(e.target.value)}
              className="w-full appearance-none bg-slate-500/5 border border-[var(--border-color)] rounded-xl pl-10 pr-8 py-3 text-xs font-black uppercase tracking-widest text-blue-600 outline-none cursor-pointer">
              {uniqueRoles.map(r => <option key={r} value={r}>{r}</option>)}
            </select>
            <Filter className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-blue-600" />
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          <button onClick={() => setActiveStage("all")}
            className={`px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border ${activeStage === "all" ? "bg-blue-600 text-white border-blue-600 shadow-sm" : "border-[var(--border-color)] text-slate-400 hover:bg-slate-500/5"}`}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            {activeStage === "all" && (
              <div className="pt-2 flex justify-center gap-3">
                <Link to="/client/haraka"
                  className="inline-flex items-center gap-2 px-6 py-3.5 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-50 transition-all shadow-md shadow-blue-600/20">
                  <Zap className="w-3.5 h-3.5 fill-current" /> Sourcing Discovery
                </Link>
