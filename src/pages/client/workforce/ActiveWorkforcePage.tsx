...
const TABS=[{key:"all",label:"All",icon:Users},{key:"employee",label:"Employees",icon:Building2},{key:"hired_full_time",label:"Partners",icon:Briefcase},{key:"hired_contract",label:"Contractors",icon:Clock}];

const TYPE_CFG:Record<MemberType,{label:string;color:string;bg:string;dot:string}>={
  employee:{label:"Employee",color:"text-[#1A1C21]",bg:"bg-slate-100 border-slate-200",dot:"bg-[#1A1C21]"},
  hired_full_time:{label:"Partner",color:"text-slate-900",bg:"bg-slate-100 border-slate-200",dot:"bg-[#1A1C21]"},
  hired_contract:{label:"Contractor",color:"text-slate-900",bg:"bg-slate-100 border-slate-200",dot:"bg-[#1A1C21]"},
};
const ONLINE_CFG:Record<OnlineStatus,{dot:string;label:string}>={
  online:{dot:"bg-[#1A1C21]",label:"Online"},away:{dot:"bg-[#1A1C21]",label:"Away"},offline:{dot:"bg-slate-400",label:"Offline"},
};
const AVAIL_CFG:Record<AvailStatus,{label:string;color:string;bg:string}>={
  available:{label:"Available",color:"text-slate-900",bg:"bg-slate-100 border-slate-200"},
  on_leave:{label:"On leave",color:"text-slate-900",bg:"bg-slate-100 border-slate-200"},
...
function ConfirmDialog({title,message,confirmLabel="Delete",onConfirm,onCancel,danger=true}:{title:string;message:string;confirmLabel?:string;onConfirm:()=>void;onCancel:()=>void;danger?:boolean;}){
  return(<AnimatePresence><motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onCancel} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[70] flex items-center justify-center p-4"><motion.div initial={{opacity:0,scale:0.95,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:16}} onClick={e=>e.stopPropagation()} className="w-full max-w-sm bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl p-6 shadow-2xl"><div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${danger?"bg-red-500/10":"bg-slate-100"}`}><Trash2 className={`w-5 h-5 ${danger?"text-red-500":"text-slate-900"}`}/></div><h3 className="text-base font-black dark:text-white tracking-tight mb-2">{title}</h3><p className="text-sm font-medium text-slate-500 dark:text-slate-400 leading-relaxed mb-6">{message}</p><div className="flex gap-3"><button onClick={onCancel} className="flex-1 py-3 border border-[var(--border-color)] text-[10px] font-black uppercase tracking-widest text-slate-500 rounded-xl hover:bg-slate-500/5 transition-all">Cancel</button><button onClick={onConfirm} className={`flex-1 py-3 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 ${danger?"bg-red-500 hover:bg-red-400":"bg-[#1A1C21] hover:bg-amber-400"}`}>{confirmLabel}</button></div></motion.div></motion.div></AnimatePresence>);
}
        ...
        <motion.div initial={{opacity:0,scale:0.95,y:20}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95}} onClick={e=>e.stopPropagation()} className="w-full max-w-lg bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl shadow-2xl overflow-hidden">
          <div className="px-7 pt-7 pb-4">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3"><MemberAvatar member={member} size="sm"/><div><p className="text-[10px] font-black uppercase tracking-widest text-[#1A1C21] mb-0.5">Contract change request</p><h2 className="text-base font-black dark:text-white tracking-tight">{rName(member)}</h2></div></div>
              <button onClick={onClose} className="p-2 rounded-xl hover:bg-slate-500/10 text-slate-400"><X className="w-4 h-4"/></button>
            </div>
            <div className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-200"><Info className="w-4 h-4 text-[#1A1C21] shrink-0 mt-0.5"/><p className="text-[11px] font-medium text-slate-500 dark:text-slate-400 leading-relaxed">{rName(member).split(" ")[0]} must <strong className="text-slate-600 dark:text-slate-300">accept</strong> these changes before they take effect.</p></div>
          </div>
          <div className="px-7 pb-0 space-y-4 max-h-[55vh] overflow-y-auto">
            <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">What to change? *</label><div className="grid grid-cols-2 sm:grid-cols-3 gap-2">{OPTS.map(o=>{const Icon=o.icon;const active=changeTypes.includes(o.key);return(<button key={o.key} type="button" onClick={()=>toggle(o.key)} className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all border ${active?"bg-[#1A1C21] text-white border-[#1A1C21] shadow-md shadow-slate-900/10":"border-[var(--border-color)] text-slate-400 hover:bg-slate-500/5"}`}><Icon className="w-3 h-3 shrink-0"/>{o.label}</button>);})} </div></div>
            ...
            {changeTypes.includes("salary")&&(<div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">New monthly salary</label><input value={form.proposed_salary} onChange={e=>upd("proposed_salary",e.target.value)} type="number" placeholder="e.g. 6000" className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/>{member.payment_monthly&&<p className="text-[10px] text-slate-400 mt-1">Current: {fmtCurrency(member.payment_monthly,member.payment_currency)}</p>}</div>)}
            {changeTypes.includes("role_title")&&(<div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">New role title</label><input value={form.proposed_role_title} onChange={e=>upd("proposed_role_title",e.target.value)} placeholder="e.g. Senior Engineer" className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/>{member.role_title&&<p className="text-[10px] text-slate-400 mt-1">Current: {member.role_title}</p>}</div>)}
            {changeTypes.includes("department")&&(<div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">New department</label><div className="relative"><select value={form.proposed_department} onChange={e=>upd("proposed_department",e.target.value)} className="w-full appearance-none bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 cursor-pointer">{["Engineering","Design","Product","Operations","Marketing","Finance","Human Resources","Sales"].map(d=><option key={d}>{d}</option>)}</select><ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none"/></div></div>)}
            {changeTypes.includes("end_date")&&(<div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">New contract end date</label><input type="date" value={form.proposed_end_date} onChange={e=>upd("proposed_end_date",e.target.value)} className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/>{member.end_date&&<p className="text-[10px] text-slate-400 mt-1">Current end: {fmtDate(member.end_date)}</p>}</div>)}
            <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Effective date</label><input type="date" value={form.proposed_start_date} onChange={e=>upd("proposed_start_date",e.target.value)} className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/></div>
            <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Message to talent *</label><textarea value={form.message} onChange={e=>upd("message",e.target.value)} rows={3} placeholder="Explain the reason for this change..." className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl p-4 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all resize-none"/></div>
          </div>
          <div className="px-7 py-5 flex gap-3"><button onClick={onClose} className="flex-1 py-3.5 border border-[var(--border-color)] text-[10px] font-black uppercase tracking-widest text-slate-500 rounded-xl hover:bg-slate-500/5 transition-all">Cancel</button><button onClick={()=>onSend({...form,change_types:changeTypes})} disabled={!isValid||sending} className="flex-1 py-3.5 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all flex items-center justify-center gap-2 shadow-md shadow-slate-900/10 disabled:opacity-40">{sending?<Loader2 className="w-3.5 h-3.5 animate-spin"/>:<Send className="w-3.5 h-3.5"/>}Send request</button></div>
        </motion.div>
      </motion.div>
    </AnimatePresence>
  );
      ...
      {history.map(h=>{const Icon=ICONS[h.change_type]??History;return(
        <div key={h.id} className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {Object.keys(h.new_values||{}).length>0&&(<div className="flex flex-wrap gap-1 mt-1">{Object.entries(h.new_values).map(([k,v]:any)=>v!=null&&<span key={k} className="text-[9px] font-black uppercase tracking-wider bg-slate-100 text-[#1A1C21] px-1.5 py-0.5 rounded-md">{k.replace(/_/g," ")}: {typeof v==="number"?fmtCurrency(v):String(v)}</span>)}</div>)}
      ...
      <motion.div initial={{x:"100%"}} animate={{x:0}} exit={{x:"100%"}} transition={{type:"spring",damping:28,stiffness:280}}
        ...
        <div className="shrink-0 flex items-center justify-between px-5 py-4 border-b border-[var(--border-color)] bg-[var(--card-bg)]">
          ...
          <div className="flex items-center gap-1 shrink-0 ml-2">
            <button onClick={()=>onDirectEdit(member)} title="Edit profile directly" className="p-2 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-[#1A1C21] transition-colors"><Pencil className="w-4 h-4"/></button>
            <button onClick={()=>onRequestChange(member)} title="Request contract change" className="p-2 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-[#1A1C21] transition-colors"><ArrowRightLeft className="w-4 h-4"/></button>
        ...
        {expiryWarning&&(<div className="shrink-0 flex items-center gap-2 px-5 py-3 bg-[#1A1C21]/5 border-b border-slate-200"><AlertTriangle className="w-4 h-4 text-slate-900 shrink-0"/><p className="text-[11px] font-black text-slate-900">Contract expires in <strong>{daysLeft} day{daysLeft!==1?"s":""}</strong> — {fmtDate(member.end_date)}</p></div>)}
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {tab==="info"?(
            <>
              ...
              {skills.length>0&&<div className="bg-slate-500/5 rounded-xl p-4 mb-3"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-3">Skills</p><div className="flex flex-wrap gap-1.5">{skills.map(s=><span key={s} className="text-[10px] font-black uppercase tracking-wider bg-slate-100 text-[#1A1C21] border border-slate-200 px-2 py-0.5 rounded-xl">{s}</span>)}</div></div>}
              <div className="grid grid-cols-2 gap-3 mb-3">
                {[{label:"Location",value:rLocation(member)||"—"},{label:"Start date",value:fmtDate(member.start_date)},{label:"Department",value:member.department||"—"},{label:"Contract end",value:member.end_date?fmtDate(member.end_date):"Ongoing"}].map(m=>(
                  <div key={m.label} className="bg-slate-500/5 rounded-xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{m.label}</p><p className={`text-sm font-black dark:text-white ${m.label==="Contract end"&&expiryWarning?"text-slate-900":""}`}>{m.value}</p></div>
                ))}
              </div>
              {email&&<div className="bg-slate-500/5 rounded-xl p-4 mb-3"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">Email</p><a href={`mailto:${email}`} className="text-sm font-black text-[#1A1C21] hover:underline break-all">{email}</a></div>}
                ...
                <div className="mb-3">
                  ...
                  <div className="space-y-2">
                    {member.projects.map(p=>(
                      <div key={p.id} className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-200">
                        <FolderKanban className="w-4 h-4 text-[#1A1C21] shrink-0 mt-0.5"/>
        ...
        <div className="shrink-0 p-4 border-t border-[var(--border-color)] bg-[var(--card-bg)] flex gap-2">
          <button onClick={()=>onRequestChange(member)} className="flex-1 flex items-center justify-center gap-2 py-3 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all shadow-md shadow-slate-900/10"><ArrowRightLeft className="w-3.5 h-3.5"/> Request change</button>
...
function MemberCard({member,onAssign,onView,onRemove}:{member:WorkforceMember;onAssign:(m:WorkforceMember)=>void;onView:(m:WorkforceMember)=>void;onRemove:(id:string)=>void;}){
  ...
  return(
    <motion.div layout initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.97}}
      className={`p-5 rounded-2xl bg-[var(--card-bg)] border shadow-sm flex flex-col gap-4 hover:border-[#1A1C21]/30 transition-all relative group ${expiring?"border-amber-500/40":"border-[var(--border-color)]"}`}>
      {expiring&&<div className="absolute top-3 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-[#1A1C21] text-white text-[9px] font-black uppercase tracking-wider px-2.5 py-1 rounded-xl shadow-md"><AlertTriangle className="w-2.5 h-2.5"/> Expires in {daysLeft}d</div>}
      ...
      <div className="space-y-1.5">
        ...
        {hasProjects?(
          <div className="flex items-start gap-1.5 pt-0.5">
            <FolderKanban className="w-3 h-3 shrink-0 text-[#1A1C21] mt-0.5"/>
            <div className="flex flex-wrap gap-1">
              {member.projects.slice(0,2).map(p=>(
                <span key={p.id} className="text-[10px] font-black text-[#1A1C21] bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded-lg truncate max-w-[110px]">{p.name}</span>
              ))}
      ...
      <div className="flex gap-2 pt-3 border-t border-[var(--border-color)]">
        <button onClick={()=>onAssign(member)} className="flex-1 py-2.5 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all flex items-center justify-center gap-1.5 shadow-md shadow-slate-900/10">
        ...
        <motion.div initial={{opacity:0,scale:0.95,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:16}} onClick={e=>e.stopPropagation()} className="w-full max-w-md bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl p-8 shadow-2xl">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              {member.projects.length>0&&<p className="text-[10px] font-bold text-[#1A1C21] mt-1">Already on {member.projects.length} project{member.projects.length>1?"s":""}</p>}
            </div>
            <button onClick={onClose} className="p-2 rounded-xl hover:bg-slate-500/10 text-slate-400"><X className="w-4 h-4"/></button>
          </div>
          {projects.length===0?(
            <div className="text-center py-8 space-y-4"><FolderKanban className="w-12 h-12 text-slate-300 mx-auto"/><div><p className="text-sm font-black dark:text-white">No active projects yet</p><p className="text-xs text-slate-400 mt-1">Create a project first.</p></div><button onClick={()=>{onClose();navigate("/client/projects");}} className="inline-flex items-center gap-2 px-6 py-3 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all shadow-md shadow-slate-900/10"><Plus className="w-3.5 h-3.5"/>Create a project</button></div>
          ):(
            <div className="space-y-4">
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Project *</label>
                <div className="relative">
                  <select value={projectId} onChange={e=>setProjectId(e.target.value)} className="w-full appearance-none bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 cursor-pointer">
              ...
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Role on project</label><input value={role} onChange={e=>setRole(e.target.value)} placeholder="e.g. Lead Developer" className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-slate-900/10 transition-all"/></div>
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={()=>onAssign({projectId,role})} disabled={!projectId||saving} className="flex-1 py-3.5 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all flex items-center justify-center gap-2 shadow-md shadow-slate-900/10 disabled:opacity-40">{saving?<Loader2 className="w-3.5 h-3.5 animate-spin"/>:<Zap className="w-3.5 h-3.5 fill-current"/>}Assign now</button>
              </div>
            </div>
          )}
        </motion.div>
        ...
        <motion.div initial={{opacity:0,scale:0.95,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:16}}
          ...
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <p className="text-xs font-bold text-[#1A1C21] mt-1">Direct admin modifications</p>
            </div>
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button onClick={()=>onSave(form)} disabled={!form.full_name.trim()||saving}
              className="flex-1 py-3.5 bg-[#1A1C21] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1A1C21] transition-all flex items-center justify-center gap-2 shadow-md shadow-slate-900/10 disabled:opacity-40">
...
export default function ActiveWorkforcePage(){
  ...
  if(loading) return <div className="w-full flex items-center justify-center py-32"><div className="flex flex-col items-center gap-4"><Loader2 className="w-8 h-8 text-[#1A1C21] animate-spin"/><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Loading workforce...</p></div></div>;
