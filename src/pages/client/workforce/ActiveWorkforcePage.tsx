...
function ConfirmDialog({title,message,confirmLabel="Delete",onConfirm,onCancel,danger=true}:{title:string;message:string;confirmLabel?:string;onConfirm:()=>void;onCancel:()=>void;danger?:boolean;}){
  return(<AnimatePresence><motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onCancel} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[70] flex items-center justify-center p-4"><motion.div initial={{opacity:0,scale:0.95,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:16}} onClick={e=>e.stopPropagation()} className="w-full max-w-sm bg-[var(--card-bg)] border border-[var(--border-color)] rounded-none p-6 shadow-2xl"><div className={`w-10 h-10 rounded-none flex items-center justify-center mb-4 ${danger?"bg-red-500/10":"bg-amber-500/10"}`}><Trash2 className={`w-5 h-5 ${danger?"text-red-500":"text-amber-500"}`}/></div><h3 className="text-base font-black dark:text-white tracking-tight mb-2">{title}</h3><p className="text-sm font-medium text-slate-500 dark:text-slate-400 leading-relaxed mb-6">{message}</p><div className="flex gap-3"><button onClick={onCancel} className="flex-1 py-3 border border-[var(--border-color)] text-[10px] font-black uppercase tracking-widest text-slate-500 rounded-none hover:bg-slate-500/5 transition-all">Cancel</button><button onClick={onConfirm} className={`flex-1 py-3 text-white text-[10px] font-black uppercase tracking-widest rounded-none transition-all flex items-center justify-center gap-2 ${danger?"bg-red-500 hover:bg-red-400":"bg-amber-500 hover:bg-amber-400"}`}>{confirmLabel}</button></div></motion.div></motion.div></AnimatePresence>);
}
  ...
  return(
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
        <motion.div initial={{opacity:0,scale:0.95,y:20}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95}} onClick={e=>e.stopPropagation()} className="w-full max-w-lg bg-[var(--card-bg)] border border-[var(--border-color)] rounded-none shadow-2xl overflow-hidden">
          <div className="px-7 pt-7 pb-4">
            ...
            <div className="flex items-start gap-3 p-3 rounded-none bg-blue-500/5 border border-blue-500/10"><Info className="w-4 h-4 text-blue-500 shrink-0 mt-0.5"/><p className="text-[11px] font-medium text-slate-500 dark:text-slate-400 leading-relaxed">{rName(member).split(" ")[0]} must <strong className="text-slate-600 dark:text-slate-300">accept</strong> these changes before they take effect.</p></div>
          </div>
          <div className="px-7 pb-0 space-y-4 max-h-[55vh] overflow-y-auto">
            <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">What to change? *</label><div className="grid grid-cols-2 sm:grid-cols-3 gap-2">{OPTS.map(o=>{const Icon=o.icon;const active=changeTypes.includes(o.key);return(<button key={o.key} type="button" onClick={()=>toggle(o.key)} className={`flex items-center gap-2 px-3 py-2.5 rounded-none text-[10px] font-black uppercase tracking-wider transition-all border ${active?"bg-blue-600 text-white border-blue-600 shadow-md shadow-blue-600/20":"border-[var(--border-color)] text-slate-400 hover:bg-slate-500/5"}`}><Icon className="w-3 h-3 shrink-0"/>{o.label}</button>);})} </div></div>
            {changeTypes.includes("member_type")&&(<div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">New contract type</label><div className="grid grid-cols-3 gap-2">{(["employee","hired_full_time","hired_contract"] as MemberType[]).map(t=>{const cfg=TYPE_CFG[t];const active=form.proposed_member_type===t;return(<button key={t} type="button" onClick={()=>upd("proposed_member_type",t)} className={`py-2.5 rounded-none text-[10px] font-black uppercase tracking-wider transition-all border text-center leading-tight ${active?`${cfg.bg} ${cfg.color} border-current`:"border-[var(--border-color)] text-slate-400 hover:bg-slate-500/5"}`}>{cfg.label}</button>);})}</div><div className="flex items-center gap-2 mt-2"><span className={`text-[9px] font-black px-2 py-0.5 rounded-none border ${TYPE_CFG[member.member_type].bg} ${TYPE_CFG[member.member_type].color}`}>{TYPE_CFG[member.member_type].label}</span><ArrowRightLeft className="w-3 h-3 text-slate-400"/><span className={`text-[9px] font-black px-2 py-0.5 rounded-none border ${TYPE_CFG[form.proposed_member_type as MemberType].bg} ${TYPE_CFG[form.proposed_member_type as MemberType].color}`}>{TYPE_CFG[form.proposed_member_type as MemberType].label}</span></div></div>)}
            {changeTypes.includes("salary")&&(<div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">New monthly salary</label><input value={form.proposed_salary} onChange={e=>upd("proposed_salary",e.target.value)} type="number" placeholder="e.g. 6000" className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/>{member.payment_monthly&&<p className="text-[10px] text-slate-400 mt-1">Current: {fmtCurrency(member.payment_monthly,member.payment_currency)}</p>}</div>)}
            {changeTypes.includes("role_title")&&(<div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">New role title</label><input value={form.proposed_role_title} onChange={e=>upd("proposed_role_title",e.target.value)} placeholder="e.g. Senior Engineer" className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/>{member.role_title&&<p className="text-[10px] text-slate-400 mt-1">Current: {member.role_title}</p>}</div>)}
            {changeTypes.includes("department")&&(<div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">New department</label><div className="relative"><select value={form.proposed_department} onChange={e=>upd("proposed_department",e.target.value)} className="w-full appearance-none bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 cursor-pointer">{["Engineering","Design","Product","Operations","Marketing","Finance","Human Resources","Sales"].map(d=><option key={d}>{d}</option>)}</select><ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none"/></div></div>)}
            {changeTypes.includes("end_date")&&(<div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">New contract end date</label><input type="date" value={form.proposed_end_date} onChange={e=>upd("proposed_end_date",e.target.value)} className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/>{member.end_date&&<p className="text-[10px] text-slate-400 mt-1">Current end: {fmtDate(member.end_date)}</p>}</div>)}
            <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Effective date</label><input type="date" value={form.proposed_start_date} onChange={e=>upd("proposed_start_date",e.target.value)} className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/></div>
            <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Message to talent *</label><textarea value={form.message} onChange={e=>upd("message",e.target.value)} rows={3} placeholder="Explain the reason for this change..." className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-none p-4 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all resize-none"/></div>
          </div>
          <div className="px-7 py-5 flex gap-3"><button onClick={onClose} className="flex-1 py-3.5 border border-[var(--border-color)] text-[10px] font-black uppercase tracking-widest text-slate-500 rounded-none hover:bg-slate-500/5 transition-all">Cancel</button><button onClick={()=>onSend({...form,change_types:changeTypes})} disabled={!isValid||sending} className="flex-1 py-3.5 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-none hover:bg-blue-500 transition-all flex items-center justify-center gap-2 shadow-md shadow-blue-600/20 disabled:opacity-40">{sending?<Loader2 className="w-3.5 h-3.5 animate-spin"/>:<Send className="w-3.5 h-3.5"/>}Send request</button></div>
        </motion.div>
      </motion.div>
    </AnimatePresence>
  );
    ...
    <div className="space-y-4">
      {history.map(h=>{const Icon=ICONS[h.change_type]??History;return(
        <div key={h.id} className="flex items-start gap-3">
          <div className="w-7 h-7 rounded-none bg-slate-500/10 flex items-center justify-center shrink-0 mt-0.5"><Icon className="w-3.5 h-3.5 text-slate-500"/></div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-black dark:text-white">{CHANGE_LABELS[h.change_type]??h.change_type}</p>
            {Object.keys(h.new_values||{}).length>0&&(<div className="flex flex-wrap gap-1 mt-1">{Object.entries(h.new_values).map(([k,v]:any)=>v!=null&&<span key={k} className="text-[9px] font-black uppercase tracking-wider bg-blue-500/10 text-blue-600 px-1.5 py-0.5 rounded-none">{k.replace(/_/g," ")}: {typeof v==="number"?fmtCurrency(v):String(v)}</span>)}</div>)}
      ...
      <motion.div initial={{x:"100%"}} animate={{x:0}} exit={{x:"100%"}} transition={{type:"spring",damping:28,stiffness:280}}
        ...
        <div className="shrink-0 flex items-center justify-between px-5 py-4 border-b border-[var(--border-color)] bg-[var(--card-bg)]">
          ...
          <div className="flex items-center gap-1 shrink-0 ml-2">
            <button onClick={()=>onRequestChange(member)} title="Request contract change" className="p-2 rounded-none hover:bg-blue-500/10 text-slate-400 hover:text-blue-600 transition-colors"><ArrowRightLeft className="w-4 h-4"/></button>
            <button onClick={()=>setConfirmRemove(true)} title="Remove" className="p-2 rounded-none hover:bg-red-500/10 text-slate-400 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4"/></button>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {tab==="info"?(
            <>
              <div className="flex flex-col items-center text-center p-5 rounded-none bg-slate-500/5 border border-[var(--border-color)] mb-4">
                ...
                <div className="flex flex-wrap justify-center gap-2 mt-3">
                  <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase px-2.5 py-1 rounded-none border ${tc.bg} ${tc.color}`}><span className={`w-1.5 h-1.5 rounded-none ${tc.dot}`}/>{tc.label}</span>
                  <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase px-2 py-1 rounded-none border ${ac.bg} ${ac.color}`}>{ac.label}</span>
                  <span className={`inline-flex items-center gap-1.5 text-[9px] font-black uppercase px-2 py-0.5 rounded-none ${oc.dot==="bg-emerald-500"?"bg-emerald-500/10 text-emerald-600":"bg-slate-500/10 text-slate-400"}`}><span className={`w-1.5 h-1.5 rounded-none ${oc.dot}`}/>{oc.label}</span>
                </div>
              </div>
              {bio&&<div className="bg-slate-500/5 rounded-none p-4 mb-3"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1"><BookOpen className="w-3 h-3"/>Bio</p><p className="text-xs font-medium text-slate-500 dark:text-slate-400 leading-relaxed">{bio}</p></div>}
              {skills.length>0&&<div className="bg-slate-500/5 rounded-none p-4 mb-3"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-3">Skills</p><div className="flex flex-wrap gap-1.5">{skills.map(s=><span key={s} className="text-[10px] font-black uppercase tracking-wider bg-blue-500/10 text-blue-600 border border-blue-500/10 px-2 py-0.5 rounded-none">{s}</span>)}</div></div>}
              <div className="grid grid-cols-2 gap-3 mb-3">
                {[{label:"Location",value:rLocation(member)||"—"},{label:"Start date",value:fmtDate(member.start_date)},{label:"Department",value:member.department||"—"},{label:"Contract end",value:member.end_date?fmtDate(member.end_date):"Ongoing"}].map(m=>(
                  <div key={m.label} className="bg-slate-500/5 rounded-none p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{m.label}</p><p className={`text-sm font-black dark:text-white ${m.label==="Contract end"&&expiryWarning?"text-amber-600":""}`}>{m.value}</p></div>
                ))}
              </div>
              {email&&<div className="bg-slate-500/5 rounded-none p-4 mb-3"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">Email</p><a href={`mailto:${email}`} className="text-sm font-black text-blue-600 hover:underline break-all">{email}</a></div>}
              <div className="bg-slate-500/5 rounded-none p-4 mb-3"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">Monthly payment</p><PaymentCell amount={member.payment_monthly} currency={member.payment_currency}/></div>
              ...
              {member.projects.length>0?(
                <div className="mb-3">
                  ...
                  <div className="space-y-2">
                    {member.projects.map(p=>(
                      <div key={p.id} className="flex items-start gap-3 p-3 rounded-none bg-blue-500/5 border border-blue-500/10">
                ...
                <div className="flex items-center gap-3 p-4 rounded-none bg-slate-500/5 border border-dashed border-[var(--border-color)] mb-3">
        ...
        <div className="shrink-0 p-4 border-t border-[var(--border-color)] bg-[var(--card-bg)] flex gap-2">
          <button onClick={()=>onRequestChange(member)} className="flex-1 flex items-center justify-center gap-2 py-3 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-none hover:bg-blue-500 transition-all shadow-md shadow-blue-600/20"><ArrowRightLeft className="w-3.5 h-3.5"/> Request change</button>
          {email&&<a href={`mailto:${email}`} className="px-4 py-3 bg-slate-500/10 text-slate-500 text-[10px] font-black uppercase tracking-widest rounded-none hover:bg-slate-500/20 transition-all flex items-center gap-2">Email</a>}
        </div>
      </motion.div>
...
function MemberCard({member,onAssign,onView,onRemove}:{member:WorkforceMember;onAssign:(m:WorkforceMember)=>void;onView:(m:WorkforceMember)=>void;onRemove:(id:string)=>void;}){
  ...
  return(
    <motion.div layout initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.97}}
      className={`p-5 rounded-none bg-[var(--card-bg)] border shadow-sm flex flex-col gap-4 hover:border-blue-500/30 transition-all relative group ${expiring?"border-amber-500/40":"border-[var(--border-color)]"}`}>
      {expiring&&<div className="absolute top-3 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-amber-500 text-white text-[9px] font-black uppercase tracking-wider px-2.5 py-1 rounded-none shadow-md"><AlertTriangle className="w-2.5 h-2.5"/> Expires in {daysLeft}d</div>}
      ...
      <div className="flex flex-wrap gap-1.5">
        <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase px-2.5 py-1 rounded-none border ${tc.bg} ${tc.color}`}><span className={`w-1.5 h-1.5 rounded-none ${tc.dot}`}/>{tc.label}</span>
        {ac&&member.availability_status!=="available"&&<span className={`inline-flex items-center gap-1 text-[9px] font-black uppercase px-2 py-1 rounded-none border ${ac.bg} ${ac.color}`}>{ac.label}</span>}
      </div>
          ...
          <div className="flex items-start gap-1.5 pt-0.5">
            ...
            <div className="flex flex-wrap gap-1">
              {member.projects.slice(0,2).map(p=>(
                <span key={p.id} className="text-[10px] font-black text-blue-600 bg-blue-500/10 border border-blue-500/10 px-1.5 py-0.5 rounded-none truncate max-w-[110px]">{p.name}</span>
              ))}
              {member.projects.length>2&&<span className="text-[10px] font-black text-slate-400 bg-slate-500/10 px-1.5 py-0.5 rounded-none">+{member.projects.length-2} more</span>}
            </div>
          </div>
        ):(
      ...
      <div className="flex gap-2 pt-3 border-t border-[var(--border-color)]">
        <button onClick={()=>onAssign(member)} className="flex-1 py-2.5 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-none hover:bg-blue-500 transition-all flex items-center justify-center gap-1.5 shadow-md shadow-blue-600/20">
          <FolderKanban className="w-3 h-3"/>{hasProjects?`Projects (${member.projects.length})`:"Assign project"}
        </button>
        <button onClick={()=>onView(member)} className="px-4 py-2.5 bg-slate-500/10 text-slate-500 text-[10px] font-black uppercase tracking-widest rounded-none hover:bg-slate-500/20 transition-all">View</button>
      </div>
  ...
  return(
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} className="fixed inset-0 bg-black/50 z-50 backdrop-blur-sm flex items-center justify-center p-4">
        <motion.div initial={{opacity:0,scale:0.95,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:16}} onClick={e=>e.stopPropagation()} className="w-full max-w-md bg-[var(--card-bg)] border border-[var(--border-color)] rounded-none p-8 shadow-2xl">
          ...
          {projects.length===0?(
            ...
            <div className="space-y-4">
              ...
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Role on project</label><input value={role} onChange={e=>setRole(e.target.value)} placeholder="e.g. Lead Developer" className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/></div>
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={()=>onAssign({projectId,role})} disabled={!projectId||saving} className="flex-1 py-3.5 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-none hover:bg-blue-500 transition-all flex items-center justify-center gap-2 shadow-md shadow-blue-600/20 disabled:opacity-40">{saving?<Loader2 className="w-3.5 h-3.5 animate-spin"/>:<Zap className="w-3.5 h-3.5 fill-current"/>}Assign now</button>
              </div>
            </div>
          )}
        </motion.div>
...
function AddMemberModal({onAdd,onClose,saving}:any){
  ...
  const upd=(k:string,v:string)=>setForm(p=>({...p,[k]:v}));
  return(<AnimatePresence><motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} className="fixed inset-0 bg-black/50 z-50 backdrop-blur-sm flex items-center justify-center p-4"><motion.div initial={{opacity:0,scale:0.95,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:16}} onClick={e=>e.stopPropagation()} className="w-full max-w-lg bg-[var(--card-bg)] border border-[var(--border-color)] rounded-none p-8 shadow-2xl max-h-[90vh] overflow-y-auto"><div className="flex items-start justify-between mb-6"><div><h2 className="text-lg font-black dark:text-white tracking-tight">Add workforce member</h2><p className="text-xs font-bold text-slate-400 mt-1">Add manually</p></div><button onClick={onClose} className="p-2 rounded-none hover:bg-slate-500/10 text-slate-400 shrink-0"><X className="w-4 h-4"/></button></div><div className="space-y-4"><div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Member type *</label><div className="grid grid-cols-3 gap-2">{(["employee","hired_full_time","hired_contract"] as MemberType[]).map(t=>{const cfg=TYPE_CFG[t];return<button key={t} type="button" onClick={()=>upd("member_type",t)} className={`py-2.5 px-2 rounded-none text-[10px] font-black uppercase tracking-wider transition-all border text-center leading-tight ${form.member_type===t?`${cfg.bg} ${cfg.color} border-current`:"border-[var(--border-color)] text-slate-400 hover:bg-slate-500/5"}`}>{cfg.label}</button>;})}</div></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{[{k:"full_name",l:"Full name *",p:"Cameron Williamson"},{k:"email",l:"Email",p:"email@company.com"},{k:"role_title",l:"Role title",p:"Senior Engineer"},{k:"location",l:"Location",p:"Remote (Ghana)"}].map(f=>(<div key={f.k}><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">{f.l}</label><input value={(form as any)[f.k]} onChange={e=>upd(f.k,e.target.value)} placeholder={f.p} className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/></div>))}</div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Start date</label><input type="date" value={form.start_date} onChange={e=>upd("start_date",e.target.value)} className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/></div><div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Monthly payment</label><input value={form.payment_monthly} onChange={e=>upd("payment_monthly",e.target.value)} placeholder="e.g. 5000" type="number" className="w-full bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/></div></div></div><div className="flex gap-3 mt-6"><button onClick={onClose} className="flex-1 py-3.5 border border-[var(--border-color)] text-[10px] font-black uppercase tracking-widest text-slate-500 rounded-none hover:bg-slate-500/5 transition-all">Cancel</button><button onClick={()=>onAdd(form)} disabled={!form.full_name.trim()||saving} className="flex-1 py-3.5 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-none hover:bg-blue-500 transition-all flex items-center justify-center gap-2 shadow-md shadow-blue-600/20 disabled:opacity-40">{saving?<Loader2 className="w-3.5 h-3.5 animate-spin"/>:<UserPlus className="w-3.5 h-3.5"/>}Add member</button></div></motion.div></motion.div></AnimatePresence>);
}
    ...
    <div className="w-full space-y-6 pb-20 p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold dark:text-white tracking-tight">Your <span className="text-blue-600">team.</span></h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={()=>setShowAddModal(true)} className="flex items-center gap-2 px-5 py-3 bg-blue-600 text-white font-black text-xs uppercase tracking-widest rounded-none hover:bg-blue-500 transition-all shadow-md shadow-blue-600/20"><UserPlus className="w-4 h-4"/> Add member</button>
        </div>
      </div>

      {expiringCount>0&&(<motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} className="flex items-center gap-3 p-4 rounded-none bg-amber-500/5 border border-amber-500/20"><AlertTriangle className="w-5 h-5 text-amber-500 shrink-0"/><div><p className="text-xs font-black text-amber-600 uppercase tracking-widest">{expiringCount} contract{expiringCount>1?"s":""} expiring within 7 days</p><p className="text-[11px] font-medium text-slate-500 mt-0.5">Review highlighted cards and send a contract extension request.</p></div></motion.div>)}

      <div className="p-4 rounded-none bg-[var(--card-bg)] border border-[var(--border-color)] shadow-sm space-y-3">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400"/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by name, role, or location..." className="w-full pl-10 pr-4 py-3 bg-slate-500/5 border border-[var(--border-color)] rounded-none text-sm font-medium outline-none focus:ring-2 ring-blue-500/20 transition-all"/></div>
          <button onClick={()=>setFiltersOpen(v=>!v)} className={`flex items-center gap-2 px-4 py-3 border rounded-none text-xs font-black uppercase tracking-widest transition-all ${filtersOpen?"bg-blue-600 text-white border-blue-600":"border-[var(--border-color)] text-slate-400 hover:bg-slate-500/5"}`}><SlidersHorizontal className="w-3.5 h-3.5"/> Filters</button>
        </div>
        <AnimatePresence>{filtersOpen&&(<motion.div initial={{height:0,opacity:0}} animate={{height:"auto",opacity:1}} exit={{height:0,opacity:0}} className="overflow-hidden"><div className="relative pt-1"><select value={filterDept} onChange={e=>setFilterDept(e.target.value)} className="w-full appearance-none bg-slate-500/5 border border-[var(--border-color)] rounded-none px-4 py-3 text-xs font-black uppercase tracking-widest text-slate-500 outline-none cursor-pointer">{depts.map(d=><option key={d}>{d}</option>)}</select><ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none"/></div></motion.div>)}</AnimatePresence>
      </div>

      <div className="flex flex-wrap gap-2">
        {TABS.map(tab=>{const Icon=tab.icon;return(<button key={tab.key} onClick={()=>setActiveTab(tab.key)} className={`flex items-center gap-2 px-4 py-2 rounded-none text-[11px] font-black uppercase tracking-widest transition-all border ${activeTab===tab.key?"bg-[var(--card-bg)] text-blue-600 border-blue-500/30 shadow-sm":"border-[var(--border-color)] text-slate-400 hover:bg-slate-500/5"}`}><Icon className="w-3.5 h-3.5"/>{tab.label}<span className="text-[9px] opacity-60">{counts[tab.key as keyof typeof counts]}</span></button>);})}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        ].map(s=>(<div key={s.label} className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-none p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{s.label}</p><p className="text-2xl font-black dark:text-white">{s.value}</p></div>))}
      </div>
      ...
      <AnimatePresence mode="popLayout">
        {filtered.length>0?(<motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4"><AnimatePresence>{filtered.map(m=><MemberCard key={m.id} member={m} onAssign={setAssignTarget} onView={setProfileTarget} onRemove={handleRemove}/>)}</AnimatePresence></motion.div>):(
          <motion.div initial={{opacity:0}} animate={{opacity:1}} className="py-24 text-center space-y-4 bg-slate-500/5 rounded-none border border-dashed border-[var(--border-color)]">
            ...
            {!search&&<button onClick={()=>setShowAddModal(true)} className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-none hover:bg-blue-500 transition-all shadow-md shadow-blue-600/20"><UserPlus className="w-3.5 h-3.5"/> Add member</button>}
          </motion.div>
        )}
      </AnimatePresence>
