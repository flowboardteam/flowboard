...
function MemberAvatar({member,size="md"}:{member:WorkforceMember;size?:"sm"|"md"|"lg"}){
  ...
  const sz=size==="lg"?"w-16 h-16 rounded-2xl text-xl":size==="sm"?"w-8 h-8 rounded-xl text-xs":"w-12 h-12 rounded-2xl text-sm";
  ...
  return(
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
        <motion.div initial={{opacity:0,scale:0.95,y:20}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95}} onClick={e=>e.stopPropagation()} className="w-full max-w-lg bg-[var(--card-bg)] border border-[var(--border-color)] rounded-[2.5rem] shadow-2xl overflow-hidden">
          <div className="px-7 pt-7 pb-4">
            ...
            <div className="flex items-start gap-3 p-3 rounded-2xl bg-blue-500/5 border border-blue-500/10"><Info className="w-4 h-4 text-blue-500 shrink-0 mt-0.5"/><p className="text-[11px] font-medium text-slate-500 dark:text-slate-400 leading-relaxed">{rName(member).split(" ")[0]} must <strong className="text-slate-600 dark:text-slate-300">accept</strong> these changes before they take effect.</p></div>
          </div>
          <div className="px-7 pb-0 space-y-4 max-h-[55vh] overflow-y-auto">
            ...
            {changeTypes.includes("member_type")&&(<div><label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">New contract type</label><div className="grid grid-cols-3 gap-2">{(["employee","hired_full_time","hired_contract"] as MemberType[]).map(t=>{const cfg=TYPE_CFG[t];const active=form.proposed_member_type===t;return(<button key={t} type="button" onClick={()=>upd("proposed_member_type",t)} className={`py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all border text-center leading-tight ${active?`${cfg.bg} ${cfg.color} border-current`:"border-[var(--border-color)] text-slate-400 hover:bg-slate-500/5"}`}>{cfg.label}</button>);})}</div><div className="flex items-center gap-2 mt-2"><span className={`text-[9px] font-black px-2 py-0.5 rounded-full border ${TYPE_CFG[member.member_type].bg} ${TYPE_CFG[member.member_type].color}`}>{TYPE_CFG[member.member_type].label}</span><ArrowRightLeft className="w-3 h-3 text-slate-400"/><span className={`text-[9px] font-black px-2 py-0.5 rounded-full border ${TYPE_CFG[form.proposed_member_type as MemberType].bg} ${TYPE_CFG[form.proposed_member_type as MemberType].color}`}>{TYPE_CFG[form.proposed_member_type as MemberType].label}</span></div></div>)}
    ...
    <div className="space-y-4">
      {history.map(h=>{const Icon=ICONS[h.change_type]??History;return(
        <div key={h.id} className="flex items-start gap-3">
          <div className="w-7 h-7 rounded-xl bg-slate-500/10 flex items-center justify-center shrink-0 mt-0.5"><Icon className="w-3.5 h-3.5 text-slate-500"/></div>
    ...
    <AnimatePresence>
      ...
      <motion.div initial={{x:"100%"}} animate={{x:0}} exit={{x:"100%"}} transition={{type:"spring",damping:28,stiffness:280}}
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {tab==="info"?(
            <>
              <div className="flex flex-col items-center text-center p-5 rounded-[1.5rem] bg-slate-500/5 border border-[var(--border-color)] mb-4">
                ...
                <div className="flex flex-wrap justify-center gap-2 mt-3">
                  <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase px-2.5 py-1 rounded-full border ${tc.bg} ${tc.color}`}><span className={`w-1.5 h-1.5 rounded-full ${tc.dot}`}/>{tc.label}</span>
                  <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase px-2 py-1 rounded-full border ${ac.bg} ${ac.color}`}>{ac.label}</span>
                  <span className={`inline-flex items-center gap-1.5 text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${oc.dot==="bg-emerald-500"?"bg-emerald-500/10 text-emerald-600":"bg-slate-500/10 text-slate-400"}`}><span className={`w-1.5 h-1.5 rounded-full ${oc.dot}`}/>{oc.label}</span>
                </div>
              </div>
              {bio&&<div className="bg-slate-500/5 rounded-2xl p-4 mb-3"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1"><BookOpen className="w-3 h-3"/>Bio</p><p className="text-xs font-medium text-slate-500 dark:text-slate-400 leading-relaxed">{bio}</p></div>}
              {skills.length>0&&<div className="bg-slate-500/5 rounded-2xl p-4 mb-3"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-3">Skills</p><div className="flex flex-wrap gap-1.5">{skills.map(s=><span key={s} className="text-[10px] font-black uppercase tracking-wider bg-blue-500/10 text-blue-600 border border-blue-500/10 px-2 py-0.5 rounded-full">{s}</span>)}</div></div>}
              <div className="grid grid-cols-2 gap-3 mb-3">
                {[{label:"Location",value:rLocation(member)||"—"},{label:"Start date",value:fmtDate(member.start_date)},{label:"Department",value:member.department||"—"},{label:"Contract end",value:member.end_date?fmtDate(member.end_date):"Ongoing"}].map(m=>(
                  <div key={m.label} className="bg-slate-500/5 rounded-2xl p-4"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">{m.label}</p><p className={`text-sm font-black dark:text-white ${m.label==="Contract end"&&expiryWarning?"text-amber-600":""}`}>{m.value}</p></div>
                ))}
              </div>
              {email&&<div className="bg-slate-500/5 rounded-2xl p-4 mb-3"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1">Email</p><a href={`mailto:${email}`} className="text-sm font-black text-blue-600 hover:underline break-all">{email}</a></div>}
              <div className="bg-slate-500/5 rounded-2xl p-4 mb-3"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">Monthly payment</p><PaymentCell amount={member.payment_monthly} currency={member.payment_currency}/></div>
              ...
              {member.projects.length>0?(
                ...
                <div className="flex items-center gap-3 p-4 rounded-2xl bg-slate-500/5 border border-dashed border-[var(--border-color)] mb-3">
...
function MemberCard({member,onAssign,onView,onRemove}:{member:WorkforceMember;onAssign:(m:WorkforceMember)=>void;onView:(m:WorkforceMember)=>void;onRemove:(id:string)=>void;}){
  ...
  return(
    <motion.div layout initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.97}}
      className={`p-5 rounded-[2rem] bg-[var(--card-bg)] border shadow-sm flex flex-col gap-4 hover:border-blue-500/30 transition-all relative group ${expiring?"border-amber-500/40":"border-[var(--border-color)]"}`}>
      {expiring&&<div className="absolute top-3 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-amber-500 text-white text-[9px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full shadow-md"><AlertTriangle className="w-2.5 h-2.5"/> Expires in {daysLeft}d</div>}
      ...
      <div className="flex flex-wrap gap-1.5">
        <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase px-2.5 py-1 rounded-full border ${tc.bg} ${tc.color}`}><span className={`w-1.5 h-1.5 rounded-full ${tc.dot}`}/>{tc.label}</span>
        {ac&&member.availability_status!=="available"&&<span className={`inline-flex items-center gap-1 text-[9px] font-black uppercase px-2 py-1 rounded-full border ${ac.bg} ${ac.color}`}>{ac.label}</span>}
      </div>
          ...
          <div className="flex items-start gap-1.5 pt-0.5">
            ...
            <div className="flex flex-wrap gap-1">
              {member.projects.slice(0,2).map(p=>(
                <span key={p.id} className="text-[10px] font-black text-blue-600 bg-blue-500/10 border border-blue-500/10 px-1.5 py-0.5 rounded-md truncate max-w-[110px]">{p.name}</span>
              ))}
              {member.projects.length>2&&<span className="text-[10px] font-black text-slate-400 bg-slate-500/10 px-1.5 py-0.5 rounded-md">+{member.projects.length-2} more</span>}
            </div>
          </div>
        ):(
  ...
  return(
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} className="fixed inset-0 bg-black/50 z-50 backdrop-blur-sm flex items-center justify-center p-4">
        <motion.div initial={{opacity:0,scale:0.95,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.95,y:16}} onClick={e=>e.stopPropagation()} className="w-full max-w-md bg-[var(--card-bg)] border border-[var(--border-color)] rounded-[2.5rem] p-8 shadow-2xl">
